...
- 5362d86: ape-shell/`apes run` erklärt bei einem pending Grant, WARUM er wartet: der
...
- 2c015d6: Async mode (`ape-shell -c` / `apes run` without `--wait`) now reports the real grant status: a grant the IdP auto-approved at creation (standing grants, YOLO) prints a short "approved automatically" confirmation with the execute command instead of the false "pending approval" block with approve URL and waiting protocol. Exit code stays 75 (unchanged).

## 1.33.2

### Patch Changes

- 0d3249a: Security: `ape-shell -c` no longer executes after an approval timeout (#1070)
...
- 309c8e8: Emit a progress line on stderr every 15 s while waiting for grant approval (#1065). Both the adapter-path `waitForGrantStatus` and the generic ape-shell session-grant poll loop now report `⏳ still waiting for approval … <n>s (grant <id-prefix>)` so callers with stall heuristics can tell an ongoing wait from a hang. stdout stays untouched; suppress with `APES_QUIET_WAIT=1`.
...
- 4b3015a: `APES_AUTH_FILE` env override for `loadAuth`/`saveAuth` (#1062)
...
- 53ffddb: Fix `ape-shell` being silently inert under shim-based installers (pnpm)
...
- Updated dependencies [76dd28c]
  - @openape/core@0.19.0
  - @openape/agent-runtime@0.2.4
  - @openape/grants@0.12.2
  - @openape/proxy@0.4.8
  - @openape/shapes@0.7.2

## 1.31.5

### Patch Changes

- 9e1f5e5: fix(cli): `apes agents list` works off-Linux (IdP is the source of truth)
...
- b4eac63: fix(cli): no stack trace on controlled exits (pending grant, usage errors)
...
- 2026edb: fix(cli): `apes run --as <user>` works off-Linux again
...
- [`f62f55f`](https://github.com/openape-ai/openape/commit/f62f55f3f34d87e79c39bce327393256a1cace2f) Thanks [@patrick-hofmann](https://github.com/patrick-hofmann)! - fix(coding): non-interactive GitHub git auth + clarify agent persona
...
- [#480](https://github.com/openape-ai/openape/pull/480) [`8d0eded`](https://github.com/openape-ai/openape/commit/8d0ededec6b4a909845c9ae5df51f6230a76634a) Thanks [@patrick-hofmann](https://github.com/patrick-hofmann)! - fix(coding): loopback LiteLLM needs no key; push via inline token URL
...
- [`9f6fdc8`](https://github.com/openape-ai/openape/commit/9f6fdc8a026884a075be824862a05517c1d33636) Thanks [@patrick-hofmann](https://github.com/patrick-hofmann)! - fix(run --as): preserve argv for wrapped commands with metacharacters
...
- [#475](https://github.com/openape-ai/openape/pull/475) [`fff44e9`](https://github.com/openape-ai/openape/commit/fff44e9d585af5a3c17c869251ffa2406ae19cdb) Thanks [@patrick-hofmann](https://github.com/patrick-hofmann)! - fix(agents code): materialize sealed capability secrets before forge calls
...
- [#474](https://github.com/openape-ai/openape/pull/474) [`8249b43`](https://github.com/openape-ai/openape/commit/8249b432231d5b3d51a1bc292ddb49bf421e3353) Thanks [@patrick-hofmann](https://github.com/patrick-hofmann)! - fix(run): keep apes diagnostics off a wrapped command's stdout for agents
...
- [#468](https://github.com/openape-ai/openape/pull/468) [`e917ad9`](https://github.com/openape-ai/openape/commit/e917ad9d70c927e32773f3a3c08684d440996cb6) Thanks [@patrick-hofmann](https://github.com/patrick-hofmann)! - Coding-agent auto-trigger (INT-1b). The cron-runner now supports deterministic `command` tasks: when a task spec carries a `command`, the runner executes it via the gated ape-shell path instead of an LLM runLoop — no model round-trip just to fire a fixed command. `runApeShell` is now exported from `@openape/apes`. The coding-agent recipe's schedule runs `apes agents code --poll-label …` so a deployed agent polls for assigned issues and works them. (Until the recipe→task `command` field is threaded through troop, the recipe schedule expresses the poll as an explicit bash command — the portable fallback.)
...
- [#429](https://github.com/openape-ai/openape/pull/429) [`edc87ef`](https://github.com/openape-ai/openape/commit/edc87efb8ac48985690318ec18b8a9d3f31b2438) Thanks [@patrick-hofmann](https://github.com/patrick-hofmann)! - `apes agents spawn` now recycles a tombstone dscl record instead of
...
- [`bfcf215`](https://github.com/openape-ai/openape/commit/bfcf215ccb54894daf89494349cdc580ae38460e) Thanks [@patrick-hofmann](https://github.com/patrick-hofmann)! - `apes agents destroy` Phase-G teardown no longer goes through a
...
- [#396](https://github.com/openape-ai/openape/pull/396) [`4293cdf`](https://github.com/openape-ai/openape/commit/4293cdf8ed89d3fea89d80bb48c1b8df1e473e3a) Thanks [@patrick-hofmann](https://github.com/patrick-hofmann)! - New agent tool: `bash`. Lets the LLM run shell commands on the agent host via `ape-shell`. Every command goes through the OpenApe DDISA grant cycle — auto-approved by a matching YOLO scope or push-notified to the owner for one-tap approval. Runs as the agent's macOS user, so file/network access is jailed to what that user can already see. Returns `stdout`, `stderr`, and `exit_code` (plus a `timed_out` flag if the wall-clock cap is hit).
...
- [#394](https://github.com/openape-ai/openape/pull/394) [`9a207dd`](https://github.com/openape-ai/openape/commit/9a207dd5e3853f7304aa80a96c64016854b1f8d7) Thanks [@patrick-hofmann](https://github.com/patrick-hofmann)! - `apes agents spawn` and `apes nest spawn` now install the ape-agent runtime by default.
