...
//
// This is the canonical home for the parser. `apps/openape-troop` and any
// owner diagnostic re-use it so the agent-email convention is defined once.
...
export function parseAgentEmail(email: string): ParsedAgentEmail | null {
  ...
  // in case of weird edge cases (shouldn't happen for valid domains).
  const ownerDomain = parts.slice(2).join('+').replace(/_/g, '.')
  if (!ownerDomain || !ownerLocalpart || !namePart) return null

  // Strip the 8-char ownerHash suffix if present so the agentName matches
  // what the user typed.
