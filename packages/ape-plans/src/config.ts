 ...
 * plans-app-specific helpers (active team selection) that commands import
 * unchanged, plus re-exports the generic helpers under the names the rest
 * of the codebase uses.
 *
 * Migration note: the old ~/.openape/auth-plans.json state structure stored
...
export function resolveTeamId(explicit?: unknown, endpointOverride?: unknown): string {
  ...
  throw new ApiError(400, 'No team', 'Pass --team <id> or run `ape-plans teams use <id>` to set a default.')
}
