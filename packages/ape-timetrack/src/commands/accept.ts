 ...
 *
 * Works with either the full URL shown by `ape-timetrack teams invite` or the
 * raw base64url token. Agents typically call this with the URL a human or
 * another agent sent to them.
 *
 * EXAMPLE
 *   $ ape-timetrack accept https://timetrack.openape.ai/invite?t=eyJhbGc...
 *   joined team 01HXX... as editor
 *
 *   $ ape-timetrack accept eyJhbGc...
 *   already a member of team 01HXX... as owner
 *
 * ERRORS
 *   410  Invite is expired, revoked, or out of uses.
 *   401  Not logged in. Run `ape-timetrack login <email>` first.
 */
export const acceptCommand = defineCommand({
  ...
  async run({ args }) {
    ...
    }>('POST', '/api/invites/accept', { endpoint: args.endpoint, body: { token } })

    if (args.json) { printJson(result); return }
    const verb = result.already_member ? 'already a member of' : 'joined'
    const ref = result.scope === 'company' ? result.company_id : result.project_id
    printLine(`${verb} ${result.scope} ${ref} as ${result.role}`)
  },
})
