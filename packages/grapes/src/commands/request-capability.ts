...
function parseCapabilityArgs(rawArgs: string[]): {
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    switch (token) {
      case '--resource':
        if (!next) throw new Error('Missing value for --resource')
        resources.push(next)
        index += 1
        break
      case '--selector':
        if (!next) throw new Error('Missing value for --selector')
        selectors.push(next)
        index += 1
        break
      case '--action':
        ...
        break
      case '--adapter':
        if (!next) throw new Error('Missing value for --adapter')
        adapter = next
        index += 1
        break
      case '--idp':
        if (!next) throw new Error('Missing value for --idp')
        idp = next
        index += 1
        break
      case '--approval':
...
export const requestCapabilityCommand = defineCommand({
  meta: {
    ...
    description: 'Request a structured CLI capability grant for shapes',
  },
  async run({ rawArgs }) {
    const auth = loadAuth()
    if (!auth) {
