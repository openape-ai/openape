...
export function frozenIssueWrite(method, path, { repository, attachments = [], actors = [], comments = [] }) {
  ...
  if (pathname === `/api/v1/orgs/${owner}` || pathname === `/org/${owner}/settings/delete` || pathname === '/user/settings/account/delete') return true
  if (comments.some(id => pathname === `/${repository}/comments/${id}` || pathname.startsWith(`/${repository}/comments/${id}/`))) return true
...
export function createArchiveProxy({ upstream, repository, attachments, actors, comments, legacyPage }) {
  ...
  return createServer((incoming, outgoing) => {
    ...
    if (frozenIssueWrite(incoming.method ?? 'GET', incoming.url ?? '/', { repository, attachments, actors, comments })) {
