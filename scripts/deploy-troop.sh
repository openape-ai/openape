#!/usr/bin/env bash
#
# Deploy apps/openape-troop to the deploy host.delta-mind.at (troop.openape.ai).
#
# Usage: ./scripts/deploy-troop.sh
#
# Requires:
...
#     via CHATTY_HOST.
#   - Passwordless sudo on the host for `systemctl restart openape-troop.service`
#     (drop a /etc/sudoers.d/openape-troop fragment scoped to user openape).
#   - Local node/pnpm, run from the monorepo root.
#   - Pre-existing systemd unit + nginx vhost on the host (one-time bootstrap;
#     see apps/openape-troop/DEPLOY.md).
#
# Release layout on the host (mirrors the other deploy scripts):
#   /home/openape/projects/openape-troop/
...
BASE="${CHATTY_BASE:-/home/openape/projects/openape-troop}"
PORT="${CHATTY_TROOP_PORT:-3010}"
...
set -euo pipefail
cd /tmp
rm -rf libsql-pkg-troop && mkdir libsql-pkg-troop && cd libsql-pkg-troop
...
echo "→ Restart openape-troop.service"
ssh -l "${USER_}" "${HOST}" "sudo systemctl restart openape-troop.service"
...
ssh -l "${USER_}" "${HOST}" "
  ...
  sudo journalctl -u openape-troop -n 30 --no-pager
  exit 1
"
...
echo
echo "✓ Deployed ${TS} to https://troop.openape.ai"
