...
test('source fence retains issue data and parent identities while PRs and Git metadata remain writable', () => {
  ...
  try {
    ...
    for (const table of ['comment', 'issue_assignees', 'issue_label', 'issue_content_history', 'project_issue', 'reaction', 'stopwatch', 'tracked_time', 'issue_dependency', 'attachment', 'label']) db.exec(`CREATE TABLE ${table}(id INTEGER PRIMARY KEY, issue_id INTEGER, dependency_id INTEGER, comment_id INTEGER, repo_id INTEGER, poster_id INTEGER)`)
