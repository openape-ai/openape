...
// App override of the module's /account page (nuxt-auth-idp registers its
// page only when the app provides none). Same passkey/SSH/consent management
// as the module page, plus the free-idp recovery sections (#462): vacation
// mode for the adaptive cooldown and the permanent recovery history.
...
async function loadCredentials() {
  ...
  try {
    credentials.value = await $fetch<Credential[]>('/api/webauthn/credentials')
  }
  catch {
    credentials.value = []
  }
...
async function handleDeleteCredential(credentialId: string) {
  ...
  try {
    ...
    error.value = (err as { data?: { statusMessage?: string } })?.data?.statusMessage ?? 'Failed to remove device'
  }
}
function formatDate(ts: number) {
  return new Date(ts).toLocaleDateString()
}
function deviceLabel(c: Credential) {
  if (c.name)
    return c.name
  if (c.deviceType === 'multiDevice')
...
async function loadSshKeys() {
  ...
  try {
    sshKeys.value = await $fetch<SshKey[]>('/api/session/ssh-keys')
  }
  catch {
    sshKeys.value = []
  }
...
async function handleAddSshKey() {
  ...
  try {
    ...
    error.value = (err as { data?: { title?: string } })?.data?.title ?? 'Failed to add SSH key'
  }
  finally {
...
async function handleDeleteSshKey(keyId: string) {
  ...
  try {
    ...
    error.value = (err as { data?: { title?: string } })?.data?.title ?? 'Failed to remove SSH key'
  }
}
function fingerprint(keyId: string) {
...
async function loadConsents() {
  ...
  try {
    consents.value = await $fetch<Consent[]>('/api/account/consents')
  }
  catch {
    consents.value = []
  }
...
async function handleRevokeConsent(clientId: string, clientName?: string) {
  ...
  try {
    ...
    error.value = (err as { data?: { title?: string } })?.data?.title ?? 'Failed to revoke access'
  }
}
</script>
  ...
  <div class="min-h-screen py-8 px-4">
    <div class="max-w-2xl mx-auto">
      ...
      <template v-else>
        ...
        <!-- Account recovery (#462): vacation shield + permanent audit -->
