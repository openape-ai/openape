...
function ecosystemContents(apesBin: string, agentName: string): string {
  void apesBin
  // On the host (macOS) the bridge .env source comes from the launchd
  // plist's EnvironmentVariables block + the wrapper script sourcing
  // ~/Library/Application Support/openape/bridge/.env. In the container
  // there's no launchd, and sudo strips PATH/env by default — pm2's
  // `env:` block is the surviving env source.
  //
  // The values are forwarded from the nest process's own env
  // (the compose `.env` is the operator's single source of truth);
  // missing keys are simply not set, which mirrors the macOS path
  // where the bridge logs a fatal if APE_CHAT_BRIDGE_MODEL is absent.
  const envForwards = [
