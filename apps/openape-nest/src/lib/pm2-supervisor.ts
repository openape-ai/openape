...
function startScriptContents(agentName: string): string {
  ...
  // Use $(whoami) instead of a hard-coded /Users/<agentName> dscl
  // lookup so this script works for both legacy agents (dscl name =
  // bare agent name) and new prefix-style agents (dscl name =
  // openape-agent-<agentName>). The `apes run --as` wrapper already
  // setuid'd us to the correct macOS user before exec'ing this
  // script, so $(whoami) returns whichever form actually exists.
  return `#!/bin/bash
