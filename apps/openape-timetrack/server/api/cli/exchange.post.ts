...
import { signTimerackCliToken } from '../../utils/cli-token'
 ...
 * with a delegation path: tokens with `aud=timetrack.openape.ai` or carrying a
...
export default defineEventHandler(async (event) => {
  ...
  // aud='timetrack.openape.ai' (delegation) pass jose's validation. We check
  // aud manually below.
  ...
  // signTimerackCliToken is the local variant of signCliToken that additionally
  ...
  // verified identically by verifyTimerackCliToken).
  const { token, expiresAt } = await signTimerackCliToken({
    email: sub,
    act,
