...
import { verifyTimerackCliToken } from './cli-token'
 ...
 * tokens carry `scope`; first-party callers (no scope) pass through
 * unchanged (behaviour-preserving). Method → required scope:
 * GET/HEAD → `<prefix>:read`, mutating → `<prefix>:write`. `<prefix>` is
 * derived from the granted scopes themselves (e.g. `timetrack:read` →
 * `timetrack`).
 */
function enforceScope(event: H3Event, caller: Caller): Caller {
  ...
  const needed = method === 'GET' || method === 'HEAD'
    ? `${prefix}:read`
    : `${prefix}:write`
...
export async function requireCaller(event: H3Event): Promise<Caller> {
  ...
  if (authHeader?.startsWith('Bearer ')) {
    ...
    if (token) {
      const cli = await verifyTimerackCliToken(token)
