...
/**
 * Timetrack-extended CLI token payload — adds optional `scope` for delegation
 ...
 */
export interface TimerackCliTokenPayload {
  iss: string
  aud: string
  typ: 'cli'
  sub: string
...
function spConfig(): { clientId: string, sessionSecret: string } {
  ...
  const clientId = cfg?.clientId || 'timetrack.openape.ai'
 ...
 *
 * Named `signTimerackCliToken` to avoid colliding with the auto-imported
 * `signCliToken` from @openape/nuxt-auth-sp.
 */
export async function signTimerackCliToken(params: {
  email: string
  act: 'human' | 'agent'
  scope?: string[]
  ttlSeconds?: number
...
/**
 * Verify a timetrack-minted CLI bearer token (HS256, self-issued).
 * Named `verifyTimerackCliToken` to avoid colliding with the auto-imported
 * `verifyCliToken` from @openape/nuxt-auth-sp.
 */
export async function verifyTimerackCliToken(token: string): Promise<TimerackCliTokenPayload | null> {
  const { clientId } = spConfig()
  try {
    ...
    return payload as unknown as TimerackCliTokenPayload
  }
  catch {
    return null
  }
