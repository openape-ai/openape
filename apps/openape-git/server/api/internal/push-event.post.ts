...
export default defineEventHandler(async (event) => {
  ...
  // Deliberately not awaited: post-receive waits for this response, so an
  // awaited mirror push would hold the user's `git push` open until the far
  // forge answers. Failures are recorded in mirror_pushes, never lost.
