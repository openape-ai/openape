...
export const issueScopes = [
  { id: 'issues:read', description: 'Read issues you may access.', grants: ['GET /api/issues', `GET ${repo}/metadata`, 'GET /api/issue-facets', `GET ${repo}/issues`, `GET ${issue}`, `GET ${record}`, `GET ${issue}/comments`, `GET ${record}/comments`, `GET ${repo}/labels`, `GET ${issue}/pulls`, 'GET /api/issue-attachments/:id'] },
  ...
  { id: 'issues:triage', description: 'Triage issues where you have maintainer access.', grants: ['POST /api/issue-preview', `PATCH ${issue}`, `PATCH ${record}`, `POST ${issue}/transfer`, `POST ${record}/transfer`, `GET ${issue}/transfer-options`, `GET ${record}/transfer-options`, `POST ${issue}/pulls`, `DELETE ${issue}/pulls/:pullId`, `GET ${repo}/issue-assignees`] },
