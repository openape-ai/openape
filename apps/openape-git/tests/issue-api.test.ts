...
beforeEach(async () => {
  ...
  router.get('/api/repos/:owner/:name/pulls/:number/issues', defineEventHandler(links.listPullIssues))
  ...
  for (const path of ['/api/repos/:owner/:name/issues/:number', '/api/issue-records/:id']) {
    router.get(`${path}/pulls`, defineEventHandler(links.listIssuePulls))
    router.post(`${path}/pulls`, defineEventHandler(links.linkIssuePull))
    router.delete(`${path}/pulls/:pullId`, defineEventHandler(links.unlinkIssuePull))
...
describe('explicit issue/pull relations', () => {
  it('deduplicates links, shows live merge state without closure, and audits removal', async () => {
    const issue = await (await call('POST', `${root}/issues`, { title: 'Fix this', body: '' })).json()
    await getDb().insert(schema.pulls).values({ id: 'pull', repoId: 'repo', number: 7, title: 'Implementation', sourceRef: 'feature', targetRef: 'main', authorEmail: owner, createdAt: 1 })
    const path = `/api/issue-records/${issue.id}/pulls`
    for (let n = 0; n < 2; n++) expect((await call('POST', path, { repository: 'owner/project', number: 7 })).status).toBe(200)
    const list = await (await call('GET', path)).json()
    expect(list.pulls).toHaveLength(1)
    expect(list.pulls[0]).toMatchObject({ title: 'Implementation', relation: 'Related', url: '/owner/project/pulls/7' })
    expect((await (await call('GET', `${root}/pulls/7/issues`)).json()).issues).toMatchObject([{ id: issue.id, title: 'Fix this' }])
    await client.execute('UPDATE pulls SET state = \'merged\', merge_sha = \'reviewed-sha\' WHERE id = \'pull\'')
    expect((await (await call('GET', path)).json()).pulls[0].state).toBe('merged')
    expect((await (await call('GET', `/api/issue-records/${issue.id}`)).json()).state).toBe('open')
    expect((await call('DELETE', `${path}/pull`)).status).toBe(200)
    expect((await (await call('GET', `${root}/pulls/7/issues`)).json()).issues).toEqual([])
    const events = (await (await call('GET', `/api/issue-records/${issue.id}`)).json()).events
    expect(events.filter((event: { action: string }) => event.action === 'pull-linked')).toHaveLength(1)
    expect(events.some((event: { action: string }) => event.action === 'pull-unlinked')).toBe(true)
  })
  it('reveals no relation when only one side is readable and requires write access on both sides', async () => {
    const issue = await (await call('POST', `${root}/issues`, { title: 'Private issue title', body: '' })).json()
    await getDb().insert(schema.repos).values({ id: 'other', owner: 'owner', name: 'other', ownerEmail: owner, createdAt: 1 })
    await getDb().insert(schema.pulls).values({ id: 'pull', repoId: 'other', number: 7, title: 'Private pull title', sourceRef: 'feature', targetRef: 'main', authorEmail: owner, createdAt: 1 })
    const path = `/api/issue-records/${issue.id}/pulls`
    await call('POST', path, { repository: 'owner/other', number: 7 })
    const issueOnly = await bearer(reader)
    expect(await (await call('GET', path, undefined, issueOnly)).json()).toEqual({ pulls: [] })
    expect((await call('POST', path, { repository: 'owner/other', number: 7 }, issueOnly)).status).toBe(403)
    await getDb().insert(schema.grants).values({ id: 'pull-reader', status: 'approved', type: 'delegation', requester: owner, targetHost: audience, audience, grantType: 'always', createdAt: 1, request: { delegate: 'pull-reader@e2e.test', audience, grant_type: 'always', scopes: ['git:read', 'repo:owner/other'] } })
    const pullOnly = await bearer('pull-reader@e2e.test')
    expect(await (await call('GET', '/api/repos/owner/other/pulls/7/issues', undefined, pullOnly)).json()).toEqual({ issues: [] })
    expect((await call('GET', path, undefined, pullOnly)).status).toBe(404)
    await getDb().insert(schema.grants).values({ id: 'issue-writer', status: 'approved', type: 'delegation', requester: owner, targetHost: audience, audience, grantType: 'always', createdAt: 1, request: { delegate: reader, audience, grant_type: 'always', scopes: ['git:write', 'repo:owner/project'] } })
    expect((await call('POST', path, { repository: 'owner/other', number: 7 }, issueOnly)).status).toBe(404)
    expect((await call('DELETE', `${path}/pull`, undefined, issueOnly)).status).toBe(404)
    expect((await call('GET', path, undefined, await bearer(owner, ['issues:triage']))).status).toBe(403)
  })
})
