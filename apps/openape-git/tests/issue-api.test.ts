...
beforeEach(async () => {
  ...
  runtimeConfig.mockReturnValue({ issuesEnabled: true, openapeSp: { clientId: audience, sessionSecret: secret, catalogOnlyScopes: issueScopes.map(scope => scope.id), manifest: { scopes: issueScopes } } })
...
describe('native issue HTTP contract', () => {
  ...
  it('rejects unauthenticated, unrelated and scope-limited callers without leaking metadata', async () => {
    ...
    expect((await call('PATCH', `${root}/issues/1`, { state: 'closed', expectedVersion: 1 }, readerToken)).status).toBe(403)
