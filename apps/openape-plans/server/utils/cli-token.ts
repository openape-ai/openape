...
/**
 * Plans-extended CLI token payload — adds optional `scope` for delegation
 ...
 */
export interface PlansCliTokenPayload {
  iss: string
  aud: string
  typ: 'cli'
  sub: string
...
function spConfig(): { clientId: string, sessionSecret: string } {
  ...
  const clientId = cfg?.clientId || 'plans.openape.ai'
 ...
 *
 * Named `signPlansCliToken` to avoid colliding with the auto-imported
 * `signCliToken` from @openape/nuxt-auth-sp.
 */
export async function signPlansCliToken(params: {
  email: string
  act: 'human' | 'agent'
  scope?: string[]
  ttlSeconds?: number
...
/**
 * Verify a plans-minted CLI bearer token (HS256, self-issued).
 * Named `verifyPlansCliToken` to avoid colliding with the auto-imported
 * `verifyCliToken` from @openape/nuxt-auth-sp.
 */
export async function verifyPlansCliToken(token: string): Promise<PlansCliTokenPayload | null> {
  const { clientId } = spConfig()
  try {
    ...
    return payload as unknown as PlansCliTokenPayload
  }
  catch {
    return null
  }
