...
import { signPlansCliToken } from '../../utils/cli-token'
 ...
 * with a delegation path: tokens with `aud=plans.openape.ai` or carrying a
...
export default defineEventHandler(async (event) => {
  ...
  // aud='plans.openape.ai' (delegation) pass jose's validation. We check aud
  // manually below.
  ...
  // signPlansCliToken is the local variant of signCliToken that additionally
  ...
  // verified identically by verifyPlansCliToken).
  const { token, expiresAt } = await signPlansCliToken({
    email: sub,
    act,
