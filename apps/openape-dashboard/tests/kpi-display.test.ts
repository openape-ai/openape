...
describe('toneForKey', () => {
  it('flags waiting work amber and empty attention green', () => {
    ...
    expect(toneForKey('mail.inbox', 20)).toBe('neutral')
    expect(toneForKey('mail.archived', 5)).toBe('done')
    ...
    expect(labelForKey('mail.inbox')).toBe('Bewusst in der Inbox')
    expect(labelForKey('mail.archived')).toBe('Zuletzt archiviert')
