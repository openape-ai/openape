...
export default defineConfig({ test: { globalSetup: ['./e2e/o365-setup.ts'], include: capture ? ['e2e/handbook-capture.test.ts'] : ['e2e/**/*.test.ts'], exclude: capture ? [] : ['e2e/handbook-capture.test.ts'], environment: 'node', fileParallelism: true, maxWorkers: 5, testTimeout: 45000, hookTimeout: 45000, retry: 0, expect: { poll: { timeout: 10000 } } } })
