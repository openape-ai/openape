...
it('shows storage limits, preserves recovery access after an unavailable worker and surfaces backup failures', async () => {
  ...
  window.pods = { language: async () => 'en' as const, data } as unknown as typeof window.pods
...
it('disables maintenance while work is active', async () => {
  window.pods = { language: async () => 'en' as const, scripts: async () => { throw new Error('No script fixture configured') }, data: async () => ({ usedBytes: 0, freeBytes: 1024 ** 3, limitBytes: 10 * 1024 ** 3, pendingDeletion: 0, busy: true, error: null }) } as unknown as typeof window.pods
  const wrapper = mount(DataManagement); await flushPromises()
