...
INSERT INTO resources_v16 SELECT * FROM resources; DROP TABLE resources; ALTER TABLE resources_v16 RENAME TO resources; PRAGMA user_version=16;`)
  before.close()
  const after = new PodDatabase(root); stores.push(after)
  expect(after.db.prepare('PRAGMA user_version').get()?.user_version).toBe(17)
