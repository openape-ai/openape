...
INSERT INTO resources_v16 SELECT * FROM resources; DROP TABLE resources; ALTER TABLE resources_v16 RENAME TO resources; DROP TABLE script_dependencies; DROP TABLE dependency_sets; DROP TABLE draft_packages; DROP TABLE dependency_domains; ALTER TABLE onboarding DROP COLUMN default_owner; ALTER TABLE pod_descriptions DROP COLUMN manual; PRAGMA user_version=16;`)
  before.close()
  const after = new PodDatabase(root); stores.push(after)
  expect(after.db.prepare('PRAGMA user_version').get()?.user_version).toBe(schemaVersion)
