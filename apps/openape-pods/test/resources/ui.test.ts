...
describe('pod resource view', () => {
  it('shows exact assigned scope and issues revision-bound revocation', async () => {
    ...
    window.pods = { runs: async () => ({ runs: [], events: [] }), getStatus: vi.fn(), onStatus: vi.fn(), workspace: async () => ({ pods: [{ id: podId, name: 'Pod', assignment: 'Read', revision: 1, lifecycle: 'paused', activeScript: null }] }), resources }
