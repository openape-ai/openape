...
it('shows grouped pods as literal labels, selects pods, collapses groups and moves using the picker', async () => {
  const workspace = vi.fn().mockResolvedValue({ pods: [pod], organization: { revision: 4, groups: [] } }); window.pods = { programs: async () => { throw new Error('No program fixture configured') }, language: async () => 'en' as const, workspace } as unknown as typeof window.pods
...
it('retains entered text on error, creates groups and confirms removal without deleting a pod', async () => {
  const workspace = vi.fn().mockRejectedValueOnce(new Error('Groups changed. Refresh and try again.')).mockResolvedValue({ pods: [pod], organization: { revision: 4, groups: [] } }); window.pods = { programs: async () => { throw new Error('No program fixture configured') }, language: async () => 'en' as const, workspace } as unknown as typeof window.pods
...
it('keeps the revision of an open form when polling receives a concurrent rename', async () => {
  const workspace = vi.fn().mockRejectedValueOnce(new Error('Groups changed. Refresh and try again.')).mockResolvedValue({ pods: [pod], organization: { revision: 5, groups: [{ ...group, name: 'Mine' }] } }); window.pods = { programs: async () => { throw new Error('No program fixture configured') }, language: async () => 'en' as const, workspace } as unknown as typeof window.pods
