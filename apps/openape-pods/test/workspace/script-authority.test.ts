...
it('migrates existing bindings without reviving stale scripts or exposing historical instructions', () => {
  ...
  const validations = store.db.prepare('SELECT * FROM validations').all()
  store.db.exec('ALTER TABLE pods DROP COLUMN metadata_revision; DROP TABLE script_dependencies; DROP TABLE dependency_sets; DROP TABLE draft_packages; DROP TABLE dependency_domains; PRAGMA user_version=15')
