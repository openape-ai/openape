...
it('recovers the reviewed creation history, preserves timestamps and rejects stale or mixed history', () => {
  ...
  try {
    const pod = store.createPod({ name: 'Existing', assignment: 'Keep unchanged' }); const conversations = new MasterConversations(store)
    ...
    const request = { action: 'create', name: pod.name, assignment: pod.assignment }
    ...
    const other = store.createPod({ name: 'Other', assignment: 'Leave' })
