...
function fixture() {
  ...
  const pod = store.createPod({ name: 'One', assignment: 'Process local files' }); const registry = new ResourceRegistry(store, () => {})
...
function version(f: ReturnType<typeof fixture>, code = 'export async function run() {}') {
  const hash = digest(code); const revision = f.store.getPod(f.pod.id).revision
...
it('requires exact-source owner approval and invalidates it after credential rotation or assignment changes', () => {
  ...
  f.store.updatePod(f.pod.id, 1, { name: 'Changed', assignment: 'New assignment', lifecycle: 'paused' }); expect(() => f.authority.approve(f.pod.id, hash, 1, 2)).toThrow()
})
it('keeps identical aliases separate across pods and rejects stale, missing or revoked assignments', () => {
  const f = fixture(); const other = f.store.createPod({ name: 'Two', assignment: 'Separate account' }); const first = randomUUID(); const second = randomUUID()
...
it('migrates schema 11 resources without losing assignments or their revisions', async () => {
  ...
  previous.exec(`DROP TABLE pod_chat_origins; DROP TABLE master_creations; DROP TABLE pod_descriptions; DROP TABLE summary_domains; DROP TABLE program_leases; DROP TABLE master_message_scopes; DROP TABLE master_contexts; DROP TABLE pod_variables; DROP TABLE script_credential_approvals;
