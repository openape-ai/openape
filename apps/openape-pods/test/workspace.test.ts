...
describe('pod workspace shell', () => {
  it('shows actual worker updates, contextual navigation and disabled execution', async () => {
    ...
    window.pods = { resources: async () => ({ resources: [], epoch: 0 }), workspace: async () => ({ pods: [] }), getStatus: async () => ready, onStatus: (callback) => { listener = callback; return unsubscribe } }
  ...
  it('surfaces IPC connection failure instead of claiming readiness', async () => {
    window.pods = { resources: async () => ({ resources: [], epoch: 0 }), workspace: async () => ({ pods: [] }), getStatus: async () => { throw new Error('Connection rejected') }, onStatus: () => () => {} }
