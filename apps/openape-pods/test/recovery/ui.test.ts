...
describe('explicit recovery UI', () => {
  it('shows unknown outcomes and requires a successful inspection before retrying', async () => {
    ...
    window.pods = { runs, workspace: vi.fn().mockResolvedValue({ pods: [{ id: podId, name: 'Fixture', assignment: 'Synthetic only', revision: 1, lifecycle: 'paused', activeScript: null }] }), scheduling: vi.fn().mockResolvedValue({ pending: 0, blocked: 1 }), resources: vi.fn(), getStatus: vi.fn(), onStatus: vi.fn() }
