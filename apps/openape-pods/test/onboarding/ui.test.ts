...
it('requires exact resource review, resets consent on assignment changes and surfaces denial', async () => {
  ...
  window.pods = { language: async () => 'en' as const, scripts: async () => { throw new Error('No script fixture configured') }, data: async () => ({ usedBytes: 0, freeBytes: 1024 ** 3, limitBytes: 10 * 1024 ** 3, pendingDeletion: 0, busy: false, error: null }), onboarding } as unknown as typeof window.pods
