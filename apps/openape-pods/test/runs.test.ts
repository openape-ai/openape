...
describe('manual runs view', () => {
  it('selects a bounded example and presents runner failures', async () => {
    ...
    window.pods = { language: async () => 'en' as const, scripts: async () => { throw new Error('No script fixture configured') }, data: async () => ({ usedBytes: 0, freeBytes: 1024 ** 3, limitBytes: 10 * 1024 ** 3, pendingDeletion: 0, busy: false, error: null }), onboarding: async () => ({ connections: [], complete: true, runtime: { ready: true, error: null } }), master: async () => ({ connected: false, state: 'idle', error: null, messages: [], drafts: [], proposals: [] }), details: async () => ({ claims: [], total: 0, counts: { finding: 0, question: 0, gap: 0 }, checkpointRevision: 0, versions: [], source: null }), scheduling: async () => ({ spec: null, enabled: false, revision: 0, nextAt: null, error: null, pending: 0, blocked: 0, concurrency: 2 }), runs, getStatus: vi.fn(), onStatus: vi.fn(), resources: vi.fn(), workspace: async () => ({ organization: { revision: 1, groups: [] }, pods: [{ id: podId, name: 'Example pod', assignment: 'Synthetic', revision: 1, lifecycle: 'paused', activeScript: null }] }) }
