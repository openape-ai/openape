...
Optional npm libraries belong to this script: expand Dependencies · package.json, declare exact versions in dependencies (for example {"dependencies":{"csv-parse":"6.1.0"}}), save, then choose Prepare dependencies. Confirm the download in the native dialog. Preparation pauses this pod and requires other work to finish. The app uses an isolated temporary HOME, public npm registry access only and no credentials or installation scripts. V1 supports pure JavaScript packages only; packages requiring native addons, executable links or installation hooks are rejected. A package set is limited to 64 MiB, 10,000 entries and 1,000 locked packages; preparation has a three-minute timeout and monitored staging storage. Regular runs never install or update packages. The full prepared tree is stored read-only, included in backups and checked before execution. Libraries run with the same permissions and secret access as your script. Changing the prepared dependency set changes the script approval. The chat can propose packages and reuse prepared sets; you prepare new sets here.
