...
writeFileSync(join(directory, 'foundation-report.html'), `<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>OpenApe Pods M1 evidence</title><style>body{font:15px/1.6 system-ui;margin:30px auto;padding:0 24px;max-width:1100px;background:#f5f6f3;color:#243027}article{background:white;padding:24px;border:1px solid #dde3da;border-radius:12px;margin:24px 0}img{max-width:100%;height:auto;border-radius:8px}h1,h2{line-height:1.3}pre,code{overflow-wrap:anywhere;white-space:pre-wrap}li{margin:12px 0}</style><h1>OpenApe Pods · Desktop foundation</h1><p>Fixture-only macOS application. Electron 40.9.3 · bundled Node 24.14.1 · arm64. This evidence does not authorize live resources or certify release signing.</p><p>Revision <code>${escape(revision)}</code>${dirty ? ' with local changes' : ''}. ${result.numPassedTests} passing / ${result.numFailedTests} failing Electron tests.</p><ul>${outcomes}</ul>${cards}</html>`)
