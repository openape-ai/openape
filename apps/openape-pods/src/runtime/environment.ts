...
export function podEnvironmentValues(root: string, podId: string): Record<string, string> {
  const home = join(root, 'pods', podId, 'home'); const bin = join(root, 'shell-launchers', podId, 'bin')
  return { HOME: home, TMPDIR: join(home, 'tmp'), PATH: `${bin}:/usr/bin:/bin`, SHELL: join(bin, 'ape-shell'), APES_TARGET_HOST: `pods:${podId}`, APES_SHELL_CLEAN_START: '1', APES_IGNORE_USER_CONFIG: '1', APE_WAIT: '1', PODS_POD_ID: podId, LANG: 'en_US.UTF-8', TERM: 'xterm-256color', ELECTRON_RUN_AS_NODE: '1' }
}
export function visibleEnvironment(environment: Record<string, string>): Record<string, string> {
  return Object.fromEntries(Object.entries(environment).filter(([key]) => ['HOME', 'TMPDIR', 'PATH', 'SHELL', 'PODS_POD_ID', 'LANG', 'TERM', 'ELECTRON_RUN_AS_NODE'].includes(key)))
}
export async function podEnvironment(root: string, podId: string, runtime: ShellRuntime): Promise<PodEnvironment> {
  ...
  return { podId, home, workspace, bin, environment: podEnvironmentValues(canonical, podId) }
}
