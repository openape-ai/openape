...
export function assignedMail(resources: PodResource[]): { mail: MailAssignment, identity: PodIdentityReference } {
  ...
  const since = parseSince(mail.since ?? null)
  ...
  return { mail: { account: mail.account, folders: mail.folders as string[], attachments: mail.attachments, ...(since ? { since } : {}), connectionId: mail.connectionId, grants: mail.grants as MailAssignment['grants'] }, identity }
}
