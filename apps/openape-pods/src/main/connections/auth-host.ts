...
async function main(): Promise<void> {
  const config = JSON.parse(await readFile(process.argv[2], 'utf8')) as { executable: string, args: string[], cwd: string, environment: Record<string, string> }
  const channel = new Socket({ fd: 3, readable: true, writable: true })
  const child = spawn(config.executable, config.args, { cwd: config.cwd, env: config.environment, stdio: ['pipe', 'pipe', 'pipe'] })
...
void main().catch((error: unknown) => { console.error(error instanceof Error ? error.message : 'Authentication host failed'); process.exitCode = 1 })
