...
export const runtimeReference = {
  ...
  jev: {
    ...
    setup: 'jevConnection reports actual native connection availability, never a key. If missing or expired, direct the owner to desktop Accounts > TypeSafe / Jev. Never request a key in chat or import it as a Pod secret. Assign in Permissions. Connected Codex can call resources with {type:assignJev,podId,epoch,connectionId,model:"jev-1.13.0",maxAttempts:20}. Existing identity grants apply. The central browser can inspect availability; native setup stays on desktop.',
