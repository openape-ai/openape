...
export const runtimeReference = {
  ...
  script: {
    ...
    mailArchive: 'Assigned pods-mail companion only: context.mail.archive.prepare({application,mailbox,items:[{id,version,reason}]}) freezes provider metadata and returns a readable, expiring once-grant URL. It never moves mail. context.mail.archive.process() checks prior decisions and executes only manually owner-approved exact batches; call from a separate polling run so pending grants do not block the briefing. Changed messages are skipped. Unknown/interrupted outcomes prohibit replay and need owner reconciliation. Generic tool calls stay read-only. Examples: examples/microsoft-mail.mjs and examples/mail-triage.mjs.',
