export const runtimeReference = {
  ...
  workflow: [
    ...
    'Save a draft, validate it, inspect the error and repair that same draft with its current revision. Activate only a successful validation; credential-reading scripts additionally require owner approval of that exact code in Settings.',
  ...
  actions: {
    runtime: {}, list: {}, create: { name: 'string', assignment: 'string' },
    inspect: { podId: 'UUID', revision: 'current assignment revision' },
    revise: { podId: 'UUID', revision: 'current assignment revision', name: 'string', assignment: 'string' },
    setVariable: { podId: 'UUID', revision: 'current assignment revision', name: 'lowercase alias', value: 'ordinary string, not a secret', variableRevision: '0 for new, otherwise current variable revision' },
    prepareSchedule: { podId: 'UUID', revision: 'current assignment revision', spec: '{kind:"interval",seconds:900} or {kind:"daily",time:"09:00",timezone:"Europe/Vienna"}', scheduleRevision: '0 for new, otherwise current schedule revision' },
    setGroup: { podId: 'UUID', revision: 'current assignment revision', name: 'existing/new group name, or null for ungrouped', organizationRevision: 'inspect.organization.revision' },
    draft: { podId: 'UUID', revision: 'current assignment revision', draftId: 'null for new, otherwise UUID', draftRevision: '0 for new, otherwise current draft revision', code: 'JavaScript ES module, up to 150000 characters', capabilities: 'exact tool.* capabilities from inspect plus credential.<alias>, at most 16' },
    validate: { podId: 'UUID', revision: 'current assignment revision', draftId: 'UUID', draftRevision: 'current draft revision' },
    activate: { podId: 'UUID', revision: 'current assignment revision', draftId: 'UUID', draftRevision: 'current draft revision' },
    run: { podId: 'UUID', revision: 'current assignment revision' },
    pause: { podId: 'UUID', revision: 'current assignment revision' },
    resume: { podId: 'UUID', revision: 'current assignment revision; requires current validated active script and never enables a disabled schedule' },
    rollback: { podId: 'UUID', revision: 'current assignment revision', hash: 'existing validated SHA256', expectedActive: 'current active SHA256 or null' },
    requestAccess: { podId: 'UUID', revision: 'current assignment revision', request: '{provider:"application",application:"name",command:"arguments",description:"reason"}, {provider:"http",origin:"https://api.example.org",description:"methods and reason"}, {provider:"reference",description:"required file"}, or {provider:"credential",alias:"bot_token",description:"purpose"}. Never include a value.' },
