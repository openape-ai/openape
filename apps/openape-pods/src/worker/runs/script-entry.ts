...
async function main(): Promise<void> {
  ...
  try {
    ...
    const result = await script.run(Object.freeze({ jev: Object.freeze({ evaluate: (payload: unknown) => request('jev.evaluate', payload) }), workflow: Object.freeze({ publish: (payload: unknown) => request('workflow.publish', payload) }), variables: Object.freeze(input.variables ?? {}), input, home: input.home ?? process.env.HOME, directories: Object.freeze((input.directories ?? []).map(item => Object.freeze(item))), workspace: input.workspace, references: input.references, signal: controller.signal, http: Object.freeze({ request: (payload: unknown) => request('http.request', payload) }), credentials: Object.freeze({ get: (alias: string) => request('credentials.get', { alias }) }), mail: Object.freeze({ archive: Object.freeze({ prepare: (proposal: unknown) => request('mail.archive', { operation: 'prepare', proposal }), process: () => request('mail.archive', { operation: 'process' }) }), workflow: Object.freeze({ filter: () => request('mail.workflow.filter', {}), remaining: (offset = 0) => request('mail.workflow.remaining', { offset }), notify: (summary: string) => request('mail.workflow.notify', { summary }) }), next: () => request('mail.next', {}), commit: (payload: unknown) => request('mail.commit', payload) }), tools: Object.freeze({ invoke: (payload: unknown) => request('tools.invoke', payload) }), agent: Object.freeze({ run: (payload: unknown) => request('agent.run', payload) }), progress: Object.freeze({ commit: (payload: unknown) => request('progress.commit', payload) }), log: (message: string) => send('log', { message }) }))
