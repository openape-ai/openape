...
async function main(): Promise<void> {
  ...
  try {
    ...
    const result = await script.run(Object.freeze({ input, workspace: input.workspace, references: input.references, signal: controller.signal, mail: Object.freeze({ next: () => request('mail.next', {}), commit: (payload: unknown) => request('mail.commit', payload) }), tools: Object.freeze({ invoke: (payload: unknown) => request('tools.invoke', payload) }), agent: Object.freeze({ run: (payload: unknown) => request('agent.run', payload) }), progress: Object.freeze({ commit: (payload: unknown) => request('progress.commit', payload) }), log: (message: string) => send('log', { message }) }))
