...
export interface MailItem { account: string, folder: string, id: string, version: string, sourceId: string, sourceHash: string, subject: string, conversationId: string, receivedAt: string, sentAt: string, hasAttachments: boolean, contentType: string, excerpt: string, truncated: boolean }
...
export async function ingestMailPage(store: PodDatabase, podId: string, root: string, artifact: MailArtifact, scope: MailScope, request: MailRead, assertCurrent: () => void): Promise<MailPage> {
  ...
  if (page.version !== 1 || page.operation !== request.operation || page.account !== scope.account || page.folder !== request.folder || !scope.folders.includes(request.folder) || (page.message ?? undefined) !== request.message || !Array.isArray(page.items) || page.items.length > 20 || typeof page.complete !== 'boolean' || (page.nextCursor !== undefined && (typeof page.nextCursor !== 'string' || page.nextCursor.length > 16384)) || page.complete !== !page.nextCursor) throw new Error('Mail response does not match the assigned read')
  ...
  const items = page.items.map((value): MailItem => {
    ...
    if (Buffer.byteLength(source) > 28 * 1024 * 1024) throw new Error('Mail item exceeds the 28 MiB source limit; record an ingestion gap')
    const version = request.operation === 'messages' ? text(row.changeKey) : digest(source)
    const sourceId = `mail:${digest(`${scope.account}\0${request.message ?? id}\0${request.operation}\0${id}\0${version}\0${digest(source)}`)}`
    ...
    return { account: scope.account, folder: request.folder, id, version, sourceId, sourceHash: hash, subject: typeof row.subject === 'string' ? row.subject.slice(0, 1024) : typeof row.name === 'string' ? row.name.slice(0, 1024) : '', conversationId: typeof row.conversationId === 'string' ? row.conversationId.slice(0, 2048) : '', receivedAt: typeof row.receivedDateTime === 'string' ? row.receivedDateTime.slice(0, 40) : '', sentAt: typeof row.sentDateTime === 'string' ? row.sentDateTime.slice(0, 40) : '', hasAttachments: row.hasAttachments === true, contentType: typeof body?.contentType === 'string' ? body.contentType.slice(0, 100) : typeof row.contentType === 'string' ? row.contentType.slice(0, 100) : '', excerpt: content.slice(0, 6000), truncated: content.length > 6000 }
  })
  assertCurrent()
  store.transaction(() => {
    store.getPod(podId)
    if (request.operation === 'messages') {
      for (const item of items) store.db.prepare('INSERT INTO mail_items VALUES(?,?,?,?,?,?,?) ON CONFLICT(pod_id,account,id) DO UPDATE SET folder=excluded.folder,source_id=excluded.source_id,conversation=excluded.conversation,metadata=excluded.metadata').run(podId, scope.account, item.id, request.folder, item.sourceId, item.conversationId, JSON.stringify(item))
    }
