...
export function installMailRecipe(store: PodDatabase, resources: ResourceRegistry, podId: string, dependencyLockHash: string): string {
  ...
  const artifact = `export const assignmentRevision=${pod.bindingRevision};\n${mailRecipe}`
  ...
  store.db.prepare('INSERT OR REPLACE INTO validations VALUES(?,?,?,?,?)').run(podId, hash, pod.bindingRevision, resources.epoch(podId), JSON.stringify({ kind: 'bundled-mail-recipe', version: 1, dependencyLockHash }))
  new WorkspaceDetails(store, resources).execute({ type: 'activate', podId, hash, assignmentRevision: pod.bindingRevision, expectedActive: pod.activeScript })
  return hash
}
