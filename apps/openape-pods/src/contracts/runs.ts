...
export interface ScriptFrame { version: 1, runId: string, sequence: number, type: 'request' | 'result' | 'error' | 'log', id?: string, operation?: 'agent.run' | 'tools.invoke' | 'progress.commit' | 'mail.next' | 'mail.commit', payload: unknown }
export function parseFrame(value: unknown, runId: string, sequence: number): ScriptFrame {
  ...
  if (frame.type === 'request' && (typeof frame.id !== 'string' || !/^[a-z0-9-]{1,100}$/i.test(frame.id) || !['agent.run', 'tools.invoke', 'progress.commit', 'mail.next', 'mail.commit'].includes(frame.operation as string))) throw new Error('Unsupported script request')
