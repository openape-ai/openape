...
export interface RunView { effects?: { key: string, runId: string }[], runs: RunRecord[], events: RunEvent[] }
export type RunCommand = { type: 'resolveHttp', podId: string, runId: string, key: string, applied: boolean, evidence: string } | { type: 'list', podId: string, runId?: string, after?: number } | { type: 'installExample', podId: string, variant: 'deterministic' | 'agent' } | { type: 'start', podId: string, expectedScript?: string } | { type: 'cancel', podId: string, runId: string } | { type: 'recover', podId: string, runId: string, action: 'inspect' | 'retry' } | { type: 'retryQueue', podId: string }
...
export interface ScriptFrame { version: 1, runId: string, sequence: number, type: 'request' | 'result' | 'error' | 'log', id?: string, operation?: 'http.request' | 'credentials.get' | 'agent.run' | 'tools.invoke' | 'progress.commit' | 'mail.next' | 'mail.commit', payload: unknown }
export function parseFrame(value: unknown, runId: string, sequence: number): ScriptFrame {
  ...
  if (frame.type === 'request' && (typeof frame.id !== 'string' || !/^[a-z0-9-]{1,100}$/i.test(frame.id) || !['http.request', 'credentials.get', 'agent.run', 'tools.invoke', 'progress.commit', 'mail.next', 'mail.commit'].includes(frame.operation as string))) throw new Error('Unsupported script request')
