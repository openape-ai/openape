...
export function parseProgramCommand(value: unknown): ProgramCommand {
  ...
  const keys: Record<ProgramCommand['type'], string[]> = { add: ['epoch', 'source'], grant: ['applicationId', 'epoch', 'argv'], start: ['applicationId', 'epoch', 'argv'], importState: ['applicationId', 'epoch'], poll: ['sessionId', 'after'], input: ['sessionId', 'data'], resize: ['sessionId', 'columns', 'rows'], close: ['sessionId'] }
