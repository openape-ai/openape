...
export interface PodResource {
  ...
  podId: string
  revision: number
  kind: 'directory' | 'reference' | 'tool' | 'connection' | 'credential'
...
export function parseResourceCommand(value: unknown, internal = false): InternalResourceCommand {
  ...
  const keys = command.type === 'pickDirectory' ? ['type', 'podId', 'epoch'] : command.type === 'changeDirectory' ? ['type', 'podId', 'id', 'revision', 'epoch', 'access'] : internal && command.type === 'assignDirectory' ? ['type', 'podId', 'path', 'access', 'epoch'] : command.type === 'assignHttp' ? ['type', 'podId', 'epoch', 'permission'] : internal && command.type === 'approveHttp' ? ['type', 'podId', 'epoch', 'permission', 'authority'] : command.type === 'saveVariable' ? ['type', 'podId', 'name', 'value', 'revision'] : command.type === 'removeVariable' ? ['type', 'podId', 'name', 'revision'] : command.type === 'saveCredential' ? ['type', 'podId', 'alias', 'value', 'epoch'] : internal && command.type === 'assignCredential' ? ['type', 'podId', 'alias', 'credentialId', 'epoch'] : command.type === 'revoke' ? ['type', 'podId', 'id', 'revision'] : internal && command.type === 'assignReference' ? ['type', 'podId', 'name', 'path'] : ['list', 'pickReference', 'snapshot'].includes(command.type as string) ? ['type', 'podId'] : []
  ...
  if (['pickDirectory', 'changeDirectory', 'assignDirectory'].includes(command.type as string)) {
    ...
    if (command.type === 'assignDirectory' && (typeof command.path !== 'string' || !command.path.startsWith('/') || /[\0\r\n\\"]/.test(command.path))) throw new Error('Invalid directory path')
  }
...
export function parseResourceState(value: unknown): ResourceState {
  ...
  for (const resource of state.resources) {
    ...
    if (typeof resource.name !== 'string' || !['directory', 'reference', 'tool', 'connection', 'credential'].includes(resource.kind) || !['ready', 'missing', 'expired', 'revoked', 'refreshRequired'].includes(resource.state) || !resource.configuration || typeof resource.configuration !== 'object') throw new Error('Invalid resource record')
