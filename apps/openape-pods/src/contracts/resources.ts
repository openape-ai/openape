...
export type ResourceCommand = { type: 'saveCredential', podId: string, alias: string, value: string, epoch: number } | { type: 'list' | 'pickReference' | 'snapshot', podId: string } | { type: 'revoke', podId: string, id: string, revision: number }
...
export function parseResourceCommand(value: unknown, internal = false): InternalResourceCommand {
  ...
  const keys = command.type === 'saveCredential' ? ['type', 'podId', 'alias', 'value', 'epoch'] : internal && command.type === 'assignCredential' ? ['type', 'podId', 'alias', 'credentialId', 'epoch'] : command.type === 'revoke' ? ['type', 'podId', 'id', 'revision'] : internal && command.type === 'assignReference' ? ['type', 'podId', 'name', 'path'] : ['list', 'pickReference', 'snapshot'].includes(command.type as string) ? ['type', 'podId'] : []
