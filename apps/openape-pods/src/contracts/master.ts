...
export type MasterAction =
  ...
  | { action: 'requestAccess', podId: string, revision: number, request: { provider: 'microsoft' | 'reference', account?: string, folders?: string[], attachments?: boolean, description: string } }
export function parseMasterAction(value: unknown): MasterAction {
  ...
  if (item.action === 'draft' && (typeof item.code !== 'string' || !item.code.trim() || item.code.length > 150000 || !Array.isArray(item.capabilities) || item.capabilities.length > 1 || item.capabilities.some(value => value !== 'mail.read'))) throw new Error('Invalid draft contract')
