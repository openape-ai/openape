...
import type { CentralClient, CentralCommand, CentralPod, CentralRuntime } from '../../contracts/central'
...
const props = defineProps<{ client: CentralClient, desktop?: boolean }>()
...
async function select(runtimeId: string, podId: string) {
  ...
  generation++; selected.value = { runtimeId, podId }; current.value = null; baseline.value = null; source.value = null
    ...
    <div v-else class="central-layout">
      <aside class="central-sidebar" :aria-label="t('Pods')">
        ...
        <section v-for="host in runtimes" :key="host.id">
          <template v-for="group in [...host.workspace.organization.groups, { id: '', name: 'Ungrouped', podIds: host.workspace.pods.filter(pod => !host.workspace.organization.groups.some(group => group.podIds.includes(pod.id))).map(pod => pod.id) }]" :key="group.id">
            <h3 v-if="group.podIds.length">
              {{ group.id ? group.name : t('Ungrouped') }}
            </h3>
            <button v-for="pod in host.workspace.pods.filter(pod => group.podIds.includes(pod.id))" :key="pod.id" class="central-pod" :aria-current="selected?.podId === pod.id ? 'true' : undefined" :disabled="busy || !pod.online" @click="select(host.id, pod.id)">
              <span>{{ pod.name }}</span><small :class="{ 'central-online': pod.online }">● {{ pod.online ? t('Online') : t('Offline') }}</small>
            </button>
          </template>
        </section>
      ...
      <main class="central-content">
        ...
        <template v-else-if="current && baseline">
          <div class="central-title">
            <div><span class="central-eyebrow">{{ t('POD · ONLINE') }}</span><h1>{{ current.pod.scripts.pod.name }}</h1><p>{{ label(current.pod.scripts.pod.lifecycle) }} · {{ current.pod.runs.runs.length }} {{ t('recent runs') }}</p></div><button :disabled="busy" @click="resetEditor">
          ...
          <fieldset :disabled="busy || !!operationId">
            ...
            <section v-if="tab === 'History'">
              <h2>{{ t('Run history') }}</h2><label>{{ t('Run') }}<select v-model="runId"><option value="">{{ t('Recent events') }}</option><option v-for="run in current.pod.runs.runs" :key="run.id" :value="run.id">{{ new Date(run.startedAt).toLocaleString() }} · {{ label(run.state) }}</option></select></label><article v-for="run in history?.runs.filter(item => !runId || item.id === runId)" :key="run.id" class="central-card">
              ...
              </article><pre v-for="event in history?.events" :key="event.sequence">{{ event.type }} · {{ new Date(event.at).toLocaleTimeString() }}
