...
export interface ScriptBuffer { view: ScriptView | null, source: ScriptSource | null, code: string, packages: string, toolCapabilities: string[], editing: boolean, busy: boolean, error: string, message: string, compare: string | null }
...
export function scriptBuffer(podId: string): ScriptBuffer {
  ...
  const buffer = reactive<ScriptBuffer>({ view: null, source: null, code: '', packages: JSON.stringify(emptyPackages(), null, 2), toolCapabilities: [], editing: false, busy: false, error: '', message: '', compare: null })
