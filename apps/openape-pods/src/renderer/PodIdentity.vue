...
export default defineComponent({
  ...
  emits: ['accounts'],
  data() { return { view: null as OnboardingView | null, busy: false, error: '', loadError: '', brokerReview: false, brokerRevoking: false, provider: { issuer: 'https://pods.openape.ai', domain: 'pods.openape.ai' }, closed: false, timer: null as ReturnType<typeof setTimeout> | null } },
  computed: {
    ...
    <template v-if="identity && !loadError">
      <template v-if="owner">
        ...
        <p v-if="!identity.subject">
          {{ t('This pod’s agent is created when you first review permissions for this pod.') }}
        </p>
        <p v-if="owner.state !== 'ready'" role="alert">
          {{ t('Sign in to your DDISA account again in Your accounts.') }}
        </p>
        ...
        <details>
          ...
          <p v-if="identity.bound">
            {{ t('This pod keeps its agent until you switch to another DDISA account.') }}
          </p>
        </details>
        <details v-if="owner.state === 'ready'" class="provider-settings" :open="!owner.broker">
          ...
          <template v-else>
            <p role="status">
              {{ t('Pods needs your permission once before it can create agents for your pods.') }}
            </p>
            ...
            <form v-if="brokerReview" class="provider-review" @submit.prevent="request({ type: 'enableBroker', id: owner.id, issuer: provider.issuer, domain: provider.domain })">
              ...
              <dl>
                <dt>{{ t('Agent provider') }}</dt><dd>{{ provider.issuer }}</dd>
                <dt>{{ t('Agent identity domain') }}</dt><dd>{{ provider.domain }}</dd>
              </dl>
      ...
      <template v-else>
        <p>{{ t('Sign in with your DDISA account before this pod’s agent is created.') }}</p>
