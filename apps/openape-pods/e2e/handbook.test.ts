...
for (const locale of ['en', 'de']) {
  it(`handbook: renders complete ${locale} offline edition with working navigation and narrow dark layout`, async () => {
    ...
    try {
      ...
      expect(await page.locator('section').count()).toBe(24); expect(await page.locator('img').count()).toBe(12)
      ...
      expect(await page.locator('section').count()).toBe(24)
    }
...
it('handbook: captures current native screens from isolated synthetic data without executing tasks', async () => {
  ...
  setup.execute({ type: 'setDefaultOwner', id: owner })
  ...
  try {
    ...
    for (const locale of ['en', 'de'] as const) {
      ...
      await identity.getByText(title('Agent provider permission', 'Erlaubnis für den Agenten-Anbieter'), { exact: true }).click()
