  ...
  try {
    ...
    for (const locale of ['de', 'en'] as const) {
      ...
      for (const [button, german, name] of [['Your accounts', 'Deine Konten', 'setup'], ['Workspace chat', 'Arbeitsbereich-Chat', 'master'], ['Data & backups', 'Daten & Sicherungen', 'data']]) {
        await page.locator('.nav-button[aria-label]').click(); await page.getByRole('main').getByRole('button', { name: locale === 'de' ? german : button, exact: true }).click()
        await page.getByRole('heading', { name: name === 'setup' ? (locale === 'de' ? 'Deine Konten' : 'Your accounts') : name === 'master' ? 'Chats' : locale === 'de' ? german : button, exact: true }).first().waitFor()
