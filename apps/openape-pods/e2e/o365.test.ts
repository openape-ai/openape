...
it.each(['invalidRefresh', 'wrongAccount', 'cancel'] as const)('fails closed for %s in a packaged TLS read', async (mode) => {
  ...
  try {
    const domain = await launchSandbox(join(dist, 'native/pods-helper'), root, { executable: resolve('.artifacts/o365-fixture/o365-cli'), workspace, readFiles: [fixture.certificate], runtimeDirectories: [], networkPorts: [fixture.proxy.port] }, ['pods', 'read', '--account', 'pod@example.invalid', '--cache-dir', workspace, '--operation', 'messages', '--folder', 'inbox'], { ...fixture.proxy.environment, PODS_CA_FILE: fixture.certificate })
