...
it.each([false, true])('onboarding: empty setup, explicit continuation and no implicit activation (packaged=%s)', async (packaged) => {
  ...
  try {
    ...
    expect(await page.locator('.setup-connection').evaluateAll(items => items.map(item => item.getAttribute('aria-label')))).toEqual(['Codex / GPT account', 'Your DDISA account'])
    await page.getByRole('button', { name: 'OpenApe account', exact: true }).waitFor()
    await page.getByRole('button', { name: 'OpenApe account', exact: true }).getByText('Not signed in', { exact: true }).waitFor()
...
it('onboarding: duplicate owner rows collapse into one account without moving existing pods (packaged)', async () => {
  ...
  const metadata = { issuer: identity.decisionIssuer, subject: identity.owner, broker: { issuer: identity.issuer, domain: 'pods.example.invalid', connectionId: identity.brokerConnectionId }, pods: { [pod.id]: { connectionId: identity.connectionId, prepared: true, identity } } }
  const insert = store.db.prepare('INSERT INTO connections VALUES(?,?,?,?,?,?)')
  insert.run(first, 'openape', identity.owner, 'ready', null, JSON.stringify(metadata))
  insert.run(randomUUID(), 'openape', identity.owner, 'failed', 'Owner identity does not match the requested account', JSON.stringify({ issuer: identity.decisionIssuer }))
  insert.run(randomUUID(), 'openape', 'other@example.invalid', 'ready', null, JSON.stringify({ issuer: identity.decisionIssuer, subject: 'other@example.invalid' }))
  store.close()
  ...
  try {
    ...
    await expect.poll(async () => (await page.evaluate(() => window.pods.onboarding({ type: 'list' }))).connections.map(item => item.account)).toEqual([identity.owner])
    expect((await page.evaluate(() => window.pods.onboarding({ type: 'list' }))).owner).toBe(first)
    await expect.poll(() => page.getByRole('button', { name: 'OpenApe account', exact: true }).textContent()).toContain(identity.owner)
    await page.getByRole('button', { name: 'OpenApe account', exact: true }).click()
    await page.getByRole('heading', { name: 'Your accounts', exact: true }).waitFor()
    expect(await page.locator('.setup-connection').count()).toBe(2)
    expect(await page.locator('.setup-view').textContent()).not.toContain('other@example.invalid')
    await mkdir(resolve('.artifacts'), { recursive: true })
    ...
    expect(await identityCard.textContent()).toContain(identity.owner)
...
it('broker provider settings: one-time consent, retained provider and readable narrow layout (packaged)', async () => {
  ...
  const owner = randomUUID(); const metadata = { issuer: 'https://id.example.invalid', subject: 'owner@example.invalid', pods: { [connectedPod.id]: { connectionId: randomUUID(), prepared: false } } }
  setup.execute({ type: 'save', connection: { id: owner, provider: 'openape', account: 'owner@example.invalid', state: 'ready', error: null }, metadata })
  store.close()
  const launch = () => electron.launch({ executablePath: resolve('release/mac-arm64/OpenApe Pods Fixture.app/Contents/MacOS/OpenApe Pods Fixture'), args: [], cwd: resolve('.'), env: { HOME: root, TMPDIR: tmpdir(), PATH: '/usr/bin:/bin', OPENAPE_PODS_FIXTURE_DIR: root, NODE_ENV: 'test' } })
  let app = await launch()
  try {
    ...
    await fresh.getByText('Pods needs your permission once before it can create agents for your pods.', { exact: true }).waitFor()
    await fresh.getByRole('button', { name: 'Allow requests from this provider', exact: true }).click()
    await fresh.getByText('Decisions remain with owner@example.invalid.', { exact: true }).waitFor()
    await fresh.getByText('https://pods.openape.ai', { exact: true }).waitFor()
    ...
    await page.getByRole('button', { name: 'App-Einstellungen', exact: true }).click(); await page.getByLabel('Sprache', { exact: true }).selectOption('en')
    await app.close()

    const reopened = new PodDatabase(root)
    try { new SetupControl(reopened, new ResourceRegistry(reopened, () => {})).execute({ type: 'save', connection: { id: owner, provider: 'openape', account: 'owner@example.invalid', state: 'ready', error: null }, metadata: { ...metadata, broker } }) }
    finally { reopened.close() }
    ...
    expect((await page.evaluate(() => window.pods.onboarding({ type: 'list' }))).connections[0]?.broker?.domain).toBe('pods.example.invalid')
