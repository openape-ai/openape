...
describe('pod-workspace concept B', () => {
  it('uses persisted pods, separates knowledge states, expands sources and opens contextual master chat', async () => {
    ...
    await page.getByRole('tab', { name: 'Settings', exact: true }).click(); await page.getByText('More options', { exact: true }).click(); await page.getByText('Execution assignment', { exact: true }).click()
    ...
    await page.locator('.lifecycle-panel form').getByRole('button', { name: 'Save pod', exact: true }).click(); await page.getByRole('status').filter({ hasText: 'Saved locally' }).waitFor()
