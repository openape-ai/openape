...
describe('pod-workspace concept B', () => {
  it('uses persisted pods, separates knowledge states, expands sources and opens contextual master chat', async () => {
    ...
    await page.getByRole('tab', { name: 'Overview', exact: true }).click(); await page.getByText('Edit description', { exact: true }).click()
    ...
    for (const [width, height] of [[1280, 800], [1060, 850], [760, 700], [560, 700], [560, 560]]) {
      ...
      for (const theme of ['light', 'dark'] as const) {
        ...
        for (const tab of ['Overview', 'Chat', 'Script', 'Permissions', 'Settings', 'History']) {
          ...
          if (tab === 'Permissions') await page.getByText('Microsoft fixture', { exact: true }).waitFor()
