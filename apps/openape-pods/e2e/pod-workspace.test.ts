...
describe('pod-workspace concept B', () => {
  it('uses persisted pods, separates knowledge states, expands sources and opens contextual master chat', async () => {
    ...
    await page.getByRole('heading', { name: 'How can I help?', exact: true }).waitFor()
    ...
    await page.locator('.directory-select').filter({ hasText: 'Reference' }).click()
    await page.getByRole('button', { name: 'Remove directory access' }).click()
    await expect.poll(() => page.locator('.directory-select').filter({ hasText: 'Reference' }).count()).toBe(0)
    ...
    await page.getByRole('tab', { name: 'History', exact: true }).click(); await page.getByRole('button', { name: 'Run now', exact: true }).click()
    await page.getByText('Local example completed (1)', { exact: true }).waitFor()
    await page.getByText('Technical details', { exact: true }).click(); await page.getByText('checkpoint', { exact: false }).first().waitFor()
    ...
    await page.getByRole('button', { name: 'New pod', exact: false }).click(); await page.getByRole('heading', { name: 'How can I help?', exact: true }).waitFor()
    await page.getByLabel('Message', { exact: true }).waitFor()
  })
