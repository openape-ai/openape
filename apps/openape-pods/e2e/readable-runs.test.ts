...
it('readable run activity: renders actual persisted states and safe environment in both languages', async () => {
  ...
  try {
    ...
    for (const language of ['en', 'de'] as const) {
      ...
      await page.locator('.run-problem').waitFor()
      expect(await page.locator('.run-problem').textContent()).toContain(language === 'en' ? 'permission service' : 'nicht autorisiert')
      expect(await page.locator('.event-list').isVisible()).toBe(false)
      expect(await page.locator('.run-timeline').textContent()).not.toContain(language === 'en' ? 'AI request' : 'KI-Anfrage')
