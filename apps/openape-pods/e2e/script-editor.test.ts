...
`
    ...
    await panel.getByRole('button', { name: 'Run', exact: true }).click()
    await page.getByRole('button', { name: 'Edited script completed successfully', exact: true }).first().waitFor()
    ...
    for (const tab of ['Overview', 'Chat', 'Script', 'Variables and secrets', 'Permissions', 'Settings', 'History']) {
      ...
      if (tab === 'History') await page.getByRole('button', { name: 'Edited script completed successfully', exact: true }).first().waitFor()
