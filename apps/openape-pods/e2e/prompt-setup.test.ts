...
it('packaged prompt setup repairs a script, configures a pod and runs through the visible chat', async () => {
  ...
  try {
    ...
    await page.getByLabel('Message', { exact: true }).fill(setupPrompt)
    const submittedPrompt = await page.getByLabel('Message', { exact: true }).inputValue()
    ...
    await page.locator('.master-message.user').scrollIntoViewIfNeeded()
