...
it('packaged prompt setup repairs a script, configures a pod and runs through the visible chat', async () => {
  ...
  try {
    ...
    await page.getByText(setupAnswer, { exact: true }).scrollIntoViewIfNeeded()
    ...
    await expect.poll(() => page.getByRole('tab', { name: 'Settings', exact: true }).getAttribute('aria-selected')).toBe('true')
    await expect.poll(() => page.getByLabel('Pod name', { exact: true }).inputValue()).toBe('Greeting')
    ...
    await page.getByRole('button', { name: 'Open Settings', exact: true }).scrollIntoViewIfNeeded()
    ...
    await page.getByRole('button', { name: 'Einstellungen öffnen', exact: true }).scrollIntoViewIfNeeded()
