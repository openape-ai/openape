...
`@openape-official-ape-agents/*` is only a **curated discovery index**.
It carries **no special trust** — capabilities are always consented
explicitly, once, regardless of where the recipe comes from. A
first-class recipe and your own repo are technically identical.
