...
import { APP_PORT, APP_URL, PEER_STORAGE_STATE, PEER_USER, STORAGE_STATE } from './constants'

// Boots a throwaway IdP plus this app and leaves two signed-in browser states
// behind — one per side of the conversation. A chat manual with a single
// account could only ever show an unanswered request.

const APP_DIR = join(dirname(fileURLToPath(import.meta.url)), '..', '..')

const SESSION_SECRET = 'docs-session-secret-at-least-32-characters-long'
const CLIENT_ID = 'chat.example.com'
const DB_FILE = '.docs-run.db'
const DDISA_MOCK_RECORDS = {
  'example.com': { version: 'ddisa1', idp: IDP_URL, mode: 'open' },
}

// The peer needs a key of its own: the shared test keypair belongs to
// TEST_USER, and the IdP maps a key to exactly one account.
const peerKeys = generateKeyPairSync('ed25519')
const PEER_PUBLIC_KEY_SSH = keyObjectToSshString(peerKeys.publicKey, PEER_USER.email)
...
export default async function globalSetup(): Promise<() => Promise<void>> {
  ...
  await bootstrapTestUser(PEER_USER)
  await bootstrapTestUserSshKey(PEER_USER.email, PEER_PUBLIC_KEY_SSH)

  await signIn(TEST_USER.email, TEST_SSH_PRIVATE_KEY, TEST_SSH_PUBLIC_KEY, STORAGE_STATE)
  await signIn(PEER_USER.email, peerKeys.privateKey, PEER_PUBLIC_KEY_SSH, PEER_STORAGE_STATE)
