...
const activeAndPending = ref<any[]>([])
const historyGrants = ref<any[]>([])
const historyCursor = ref<string | null>(null)
const historyHasMore = ref(false)
const loadingHistory = ref(false)
const loading = ref(true)
const actionError = ref('')
const grantTypeSelections = ref<Record<string, string>>({})
const durationPresetSelections = ref<Record<string, string>>({})
const customDurations = ref<Record<string, number>>({})
const extendModeSelections = ref<Record<string, string>>({})
const similarGrantsData = ref<Record<string, any>>({})
...
async function loadSimilarGrants(grantId: string) {
  if (similarGrantsData.value[grantId]) return
  try {
    const res = await fetch(`/api/grants/${grantId}`, { credentials: 'include' })
    if (!res.ok) return
...
function hasSimilar(grantId: string) {
  return !!similarGrantsData.value[grantId]?.similar_grants?.length
}

function getEffectiveDuration(grantId: string) {
...
async function loadGrants() {
  ...
  try {
    const [activeRes, historyRes] = await Promise.all([
      fetch('/api/grants?section=active', { credentials: 'include' }).then(r => r.json()),
      fetch('/api/grants?section=history', { credentials: 'include' }).then(r => r.json()),
    ])
...
async function loadMoreHistory() {
  ...
  try {
    const res = await fetch(`/api/grants?section=history&cursor=${historyCursor.value}`, { credentials: 'include' })
    const data = await res.json()
    historyGrants.value = [...historyGrants.value, ...data.data]
    historyCursor.value = data.pagination?.cursor ?? null
    historyHasMore.value = data.pagination?.has_more ?? false
  }
  catch {
    // ignore
  }
  ...
  try {
    ...
    const extendBody = extendMode && extendMode !== 'separate' && similar?.similar_grants?.length
      ? {
          ...
          extend_grant_ids: similar.similar_grants.map((s: any) => s.grant.id),
        }
      : {}
    ...
    if (!res.ok) {
      ...
      throw new Error((err as Record<string, string>).statusMessage ?? 'Failed to approve grant')
    }
    await loadGrants()
  }
  catch (err) {
...
async function denyGrant(id: string) {
  ...
  try {
    ...
    if (!res.ok) throw new Error('Failed to deny grant')
    await loadGrants()
  }
  catch (err) {
            ...
            <UCard v-for="grant in historyGrants" :key="grant.id" class="opacity-75">
              <div class="text-sm space-y-1">
                <div class="flex flex-wrap items-center gap-2">
                  <span class="font-mono text-xs text-dimmed">{{ grant.id.slice(0, 8) }}...</span>
                  <UBadge
                    :color="({ denied: 'error', revoked: 'neutral', expired: 'warning', used: 'info', approved: 'success' } as any)[grant.status] || 'neutral'"
