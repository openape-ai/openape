...
export default defineEventHandler(async (event) => {
  ...
  const body = await readBody<{ label?: string, prompt?: string, includePayload?: boolean, enabled?: boolean, eventFilter?: string }>(event)
  const patch: Record<string, unknown> = {}
  if (typeof body?.label === 'string') patch.label = body.label.trim()
  if (typeof body?.prompt === 'string') patch.prompt = body.prompt.trim()
  if (typeof body?.eventFilter === 'string') patch.eventFilter = body.eventFilter.trim()
