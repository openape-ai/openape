...
export default defineEventHandler(async (event) => {
  ...
  if ('cronExpr' in (body ?? {})) {
    const c = typeof body.cronExpr === 'string' && body.cronExpr.trim() ? body.cronExpr.trim() : null
    if (c != null && !isValidCron(c)) throw createError({ statusCode: 400, statusMessage: 'invalid cron expression' })
    patch.cronExpr = c
  }
