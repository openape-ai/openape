...
export default defineEventHandler(async (event) => {
  ...
  return rows.map(h => ({ id: h.id, label: h.label, token: h.token, secret: h.secret, prompt: h.prompt, includePayload: h.includePayload, enabled: h.enabled, lastFiredAt: h.lastFiredAt }))
})
