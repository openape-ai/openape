...
export default defineEventHandler(async (event) => {
  ...
  return rows.map(s => ({ id: s.id, kind: s.kind, prompt: s.prompt, atHour: s.atHour, everyMinutes: s.everyMinutes, fireAt: s.fireAt, cronExpr: s.cronExpr, enabled: s.enabled, notify: s.notify, createdBy: s.createdBy, lastRunAt: s.lastRunAt }))
})
