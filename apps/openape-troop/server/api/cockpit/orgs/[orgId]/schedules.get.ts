...
export default defineEventHandler(async (event) => {
  ...
  return rows.map(s => ({ id: s.id, kind: s.kind, prompt: s.prompt, atHour: s.atHour, everyMinutes: s.everyMinutes, fireAt: s.fireAt, enabled: s.enabled, lastRunAt: s.lastRunAt }))
})
