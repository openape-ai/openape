...
export default defineEventHandler(async (event) => {
  ...
  const flat = rows.map(r => ({ id: r.id, role: r.role, label: r.label, duties: r.duties, procedure: r.procedure, vars: r.vars, tools: r.tools, enabled: r.enabled, reportsTo: r.reportsTo }))
