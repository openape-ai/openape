export interface FlatRole { id: string, role: string, label: string, duties: string, tools: string[], enabled: boolean, reportsTo: string | null }
export interface OrgNode { id: string, role: string, label: string, duties: string, tools: string[], enabled: boolean, children: OrgNode[] }
...
export function buildOrgTree(rows: FlatRole[]): OrgNode[] {
  const nodes = new Map<string, OrgNode & { reportsTo: string | null }>(
    rows.map(r => [r.id, { id: r.id, role: r.role, label: r.label, duties: r.duties, tools: r.tools, enabled: r.enabled, reportsTo: r.reportsTo, children: [] }]),
  )
