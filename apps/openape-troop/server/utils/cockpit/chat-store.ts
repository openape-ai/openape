...
export async function saveProgressChatMessage(orgId: string, owner: string, taskId: string, content: string) {
  const now = Date.now()
  const rows = await useDb().select().from(cockpitChatMessages)
    .where(and(eq(cockpitChatMessages.ownerEmail, owner), eq(cockpitChatMessages.orgId, orgId)))
    .orderBy(desc(cockpitChatMessages.createdAt))
