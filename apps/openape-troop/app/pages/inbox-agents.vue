...
const { t } = useI18n()
useSeoMeta({ title: () => t('records.tabTitle') })
...
<template>
  <div class="min-h-dvh bg-zinc-950 text-zinc-100">
    <AppHeader active="inbox" :title="t('records.tabTitle')" :show-logout="!!user" @logout="logout" />

    <main class="max-w-3xl mx-auto px-4 sm:px-8 py-8">
      <InlineLogin v-if="!user" :hint="t('common.loginHint', { what: t('records.loginWhat') })" />
      <template v-else>
        <MetricTiles :metrics="data.metrics" class="mb-6" />
        <p class="text-zinc-400 mb-4 text-sm">
          {{ t('records.fromEvents', { count: data.events_considered }, data.events_considered) }}
        </p>
