...
const COLUMNS = [
  { key: 'planned', label: 'companyPanels.objectives.column.planned' },
  { key: 'in_progress', label: 'companyPanels.objectives.column.inProgress' },
  { key: 'done', label: 'companyPanels.objectives.column.done' },
] as const
...
<template>
  <div>
    <div class="flex gap-2 mb-6">
      <UInput v-model="form.title" :placeholder="t('companyPanels.objectives.placeholder')" class="flex-1" :ui="{ base: 'w-full' }" @keydown.enter="add" />
      <UButton color="primary" icon="i-lucide-plus" :loading="saving" :disabled="!form.title.trim()" @click="add">
        {{ t('companyPanels.objectives.addButton') }}
      </UButton>
    </div>
    ...
    <div v-else class="grid gap-4 md:grid-cols-3">
      <div v-for="col in COLUMNS" :key="col.key">
        <h4 class="text-xs font-semibold uppercase tracking-wider text-zinc-500 mb-3">
          {{ t(col.label) }} <span class="text-zinc-600">({{ byStatus(col.key).length }})</span>
        </h4>
        <div class="space-y-2">
          <div
          ...
          >
            <div class="flex items-start justify-between gap-2">
              ...
              <UButton color="neutral" variant="ghost" size="xs" icon="i-lucide-x" :aria-label="t('common.remove')" :loading="busy[o.id]" @click="remove(o.id)" />
            </div>
            <div class="flex gap-1 mt-2">
              <UButton v-if="col.key !== 'planned'" color="neutral" variant="soft" size="xs" icon="i-lucide-chevron-left" :aria-label="t('companyPanels.objectives.moveLeftAria')" :loading="busy[o.id]" @click="patch(o.id, { status: col.key === 'done' ? 'in_progress' : 'planned' })" />
              <UButton v-if="col.key !== 'done'" color="neutral" variant="soft" size="xs" icon="i-lucide-chevron-right" :aria-label="t('companyPanels.objectives.moveRightAria')" :loading="busy[o.id]" @click="patch(o.id, { status: col.key === 'planned' ? 'in_progress' : 'done' })" />
            </div>
          </div>
