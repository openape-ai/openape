...
Test-pyramid steps 1–3 are merged (PR 85 `80d62c57`, PR 86 `96b39096`). M2 (delivery, recovery, shared editing): [PR 87](https://repos.openape.ai/patrick/monorepo/pulls/87) (`feature/issue-1362-m2-delivery`, source `483ddcdd`) carries generation rotation, pairing projection, Retry-After and recovery tests; `feature/issue-1362-m2-delivery-2` (stacked, rebase after PR 87 merges) adds per-runtime replay bounds, the real-stack rotation round trip and idempotent claims. Next: merge PR 87, push and review the second set, then program-effect replay proof, review-conflict UI, native RelayClient tests, and the M2 acceptance run before M3.
