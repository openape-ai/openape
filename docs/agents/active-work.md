...
Follow-up requested September 24: [PR 115](https://repos.openape.ai/patrick/monorepo/pulls/115), branch `feature/issue-1377-pod-secrets-cli-help` in worktree `issue-1375-completion-docs`, removes per-script secret selection and declaration gates. Every ready secret assigned to a Pod is available to its scripts; run lease, binding, epoch and revocation checks remain. MCP runtime help documents CLI assignment, adapter operations, grants, network hosts, private login and runtime dependencies; `program prepare` returns command metadata without execution. Source `9c74d903` passes full lint/typecheck, app build, 437 unit/component, 109 packaged and 20 browser tests, plus full `pnpm check:ci` (`1790247398618-9c74d903-all`). [Inspected synthetic UI evidence](https://testrun.openape.ai/r/4mIMTbY13J-Agov2HLDXH4w8). The initial IPC serialization regression was fixed and verified in the packaged credential flow. Main `f80190da` is green after one retry of an unrelated Git issue-page navigation timeout.

The actual IURIO Pod has `az` assigned with version, account inspection, device-login and repository-specific PR-list grants. Azure CLI 2.83.0 starts with an isolated HOME; the DevOps extension is absent there. Terminal sessions must be reopened after assignment changes. Scheduled Azure CLI execution and live monitor acceptance remain unverified; do not copy owner authentication stores or enable the schedule from synthetic validation alone. The signed delivery's source and verification receipts are kept in `~/Downloads/OpenApe-Pods-9c74d903/`; the issue records the final exact-head merge gate and installation receipt. Reinspect `~/Downloads/IURIO-PR-monitor/TASK.md` for the remaining Azure input/runtime work before continuing live acceptance.

[Issue 1377](https://repos.openape.ai/patrick/monorepo/issues/1377), [approved plan](https://plans.openape.ai/teams/01KPV1XN2S4FEGHFVPR3ZZ7VN1/plans/01M3971H91PDY5XPHJP3696PZ7). [PR 113](https://repos.openape.ai/patrick/monorepo/pulls/113) merged as `56eb0366` after exact-source CI/E2E/layout passed for `29447001`. Connected local Codex administers Pods directly; Codex governs its own confirmations. Desktop chat and the Prepared by Codex queue are removed, while history is retained. Source and merge have identical trees. Full `pnpm check:ci` passed (`1790239240109-29447001-all`), including 436 Pods unit/component, 109 packaged and 20 browser cases. [Inspected synthetic evidence](https://testrun.openape.ai/r/z7Gh5qVdFwQdXTRnaQnVUQ4A).

The signed/notarized internal build is installed at `~/Applications/OpenApe Pods.app`; the mounted DMG passed isolated execution and Gatekeeper checks. All six existing Pods retain their names, revisions and lifecycle. App/profile rollback pair: `~/Library/Application Support/OpenApe Pods Rollback/2026-09-24-105317-issue-1377`. Distribution and verification receipts: `~/Downloads/OpenApe-Pods-29447001/`. The real Codex configuration was not edited. Restart Codex to reload the expanded MCP schema if it still exposes the old tool list.

Installed MCP directly saved the IURIO description and confirmed Telegram destination, assigned Azure GET and Telegram POST with authentic owner grants, and retired the obsolete variable proposal. The existing `iurio` group and Pod are reused; draft revision 2 includes the assigned HTTP capabilities. The one-time Pods provider connection was established through the existing account flow. **Still pending:** the dedicated Azure credential requested through OpenApe Secrets, then validation/activation, a real poll and Telegram receipt, and enabling the existing 900-second schedule. The monitor remains paused; no live poll or Telegram delivery is claimed. Current preparation: `~/Downloads/IURIO-PR-monitor/TASK.md`.
