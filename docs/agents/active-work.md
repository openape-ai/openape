...
Follow-up requested September 24: branch `feature/issue-1377-pod-secrets-cli-help` removes per-script secret selection and declaration gates. Every ready secret assigned to a Pod is available to its scripts; run lease, binding, epoch and revocation checks remain. MCP runtime help documents CLI assignment, adapter operations, grants, network hosts, private login and runtime dependencies; `program prepare` returns command metadata without execution. Full lint/typecheck, app build, 437 unit/component and 20 browser tests pass. The actual IURIO Pod now has `az` assigned with version/device-login grants. Azure CLI 2.83.0 starts with an isolated HOME; the DevOps extension is absent there. Scheduled Azure CLI execution and live monitor acceptance remain unverified. Main `f80190da` has an unrelated Git issue-page navigation timeout in layout; resolve that gate before merging the follow-up.
