...
[Issue 1377](https://repos.openape.ai/patrick/monorepo/issues/1377), [proposed plan](https://plans.openape.ai/teams/01KPV1XN2S4FEGHFVPR3ZZ7VN1/plans/01M3971H91PDY5XPHJP3696PZ7). Patrick superseded the issue-1375 app-side approval requirement: connected local Codex should administer Pods directly, with any tool confirmation governed by Codex. The plan proposes removing the embedded chat UI, retaining management screens and history, and adding direct resource/schedule operations and opaque secret import through existing services. Branch `feature/issue-1377-codex-owner-control`, reused own worktree `issue-1375-completion-docs`, base `29f77d29`. Plan approved by Patrick. Direct owner commands, private secret import, description editing and removal of desktop chat/review navigation are implemented. Local full lint/typecheck, app build, 436 unit/component cases, 20 browser cases and two focused packaged MCP acceptance cases pass; actual Codex executes a fixture run without app review. Final-source PR/CI and signed installation are next. No owner profile changes yet. Preserve the existing `iurio` group and paused monitor; it still needs Azure access, resource bindings, script activation and an enabled 15-minute schedule before live acceptance.
