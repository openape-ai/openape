...
PR 116 is merged as `4f5977cb`, exact-source CI/E2E/layout and full local checks green. Pods identity provider `prod-4f5977cb` is deployed with unchanged discovery/JWKS and a verified database backup. Signed/notarized source `db63669d` is installed; all six Pods are preserved. Receipts: `~/Downloads/OpenApe-Pods-db63669d/`; paired rollback: `~/Library/Application Support/OpenApe Pods Rollback/2026-09-24-153502-issue-1377`.

Live acceptance exposed an independent replacement defect: historical run approval lookup wins over a newly owner-assigned program grant, so the executor correctly rejects the historical adapter digest. Follow-up branch `bugfix/issue-1377-assigned-grant` makes the current assignment authoritative and consults renewed run authority only when the original is absent, consumed or expired. Revoked/denied current assignments still fail closed. IURIO resource epoch 17 has the runtime, command grants and network hosts; run `45f467f2-b8bc-4e19-ae1a-afe0ae93fc27` completed with gaps, without false revocation. A separate real account/REST diagnostic `c449b30a-51a9-40d2-b3ae-f062afd3d2bc` then completed: the existing private Azure login and IURIO PR access work. No repeat login is needed. Follow-up source commit `148e9655` passes lint/typecheck, build, nine focused authorization cases and the affected commit gate. Its push failed when DNS/HTTPS connectivity disappeared on the Mac (also affects apple.com). The complete monitor run `3848b3e4-f7a5-46b4-bbd3-26dd0f860a17` timed out before its script started; no Telegram effect exists. After network recovery: push/review the grant-selection follow-up, retry the real monitor, verify delivery and continuing runtime permission, then enable the existing 900-second schedule.

The actual IURIO diagnostic failed at `az version` with exit 71: Homebrew's launcher requires Bash/Python, while the assigned-program sandbox allowed only the launcher executable. An isolated empty-HOME reproduction confirmed the same failure independently of grants. The repair adds an explicit runtime descriptor with native interpreter, fixed arguments, read-only package directories and non-secret environment; terminal and scheduled calls share it. The prototype now runs Azure CLI 2.83.0 and Azure DevOps extension 1.0.8 in the native sandbox, without owner credentials. A native regression verifies private-state reuse and continued denial of outside reads and child processes.

The separate cancellation was also diagnosed: the real runtime grant was consumed (`used`, type `once`); the brokered Pod status endpoint treated it as inactive, unlike the direct endpoint. The repair aligns active-operation checks while preserving fresh consumption for new runs, requester/key/owner checks, revocation and expiry. It requires a Pods identity-provider deployment as well as the desktop update. Follow the existing deployment and exact-head CI gates; do not enable the monitor from synthetic checks.

Delivered prerequisites: PR 113 (`56eb0366`) removed embedded chat and duplicate local Codex approvals. PR 115 (`b17e2b54`) removed per-script secret selection and added MCP CLI setup help. The previous signed/notarized installation was source `9c74d903`; six Pods and owner Codex settings were preserved. Receipts: `~/Downloads/OpenApe-Pods-9c74d903/`. Paired rollback: `~/Library/Application Support/OpenApe Pods Rollback/2026-09-24-131154-issue-1377`.

Reuse group `iurio` and Pod `98c32f74-ffaf-4628-bd41-95cea821572f`. The owner completed Azure login in its private terminal and exited; account/PR access is not yet verified. The diagnostic script is active but the Pod remains paused and its 900-second schedule disabled. Original monitor draft and Telegram configuration are preserved. Current exact IDs, runtime descriptor and pending live acceptance: `~/Downloads/IURIO-PR-monitor/TASK.md`. Do not request the same login again without new evidence; never copy owner login stores.
