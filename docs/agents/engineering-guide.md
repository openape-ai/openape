# OpenApe engineering guide

Detailed guidance, preserved from the former `.claude/CLAUDE.md` and updated
on 2026-09-07. Start with [AGENTS.md](../../AGENTS.md), the generated workspace
map and the current check contract. Historical decisions are dated below.
...
pnpm turbo run test --filter=@openape-git/app
```

## Publish Flow

Version packages on a feature branch with Changesets, then merge the version PR
with all required checks. Publish only the clean canonical main version using
`pnpm release:local --dry-run` followed by `pnpm release:local`.
See [the rollout guide](../operations/ai-workflow.md) and
[repository configuration](../../.openape/repository.json).
The release script never commits or pushes. Do not publish an unmerged version.
...
4. **Alle Änderungen über Branch + PR.** Der serverseitige Branchschutz gilt auch für Konfigurationsänderungen und Admin-Pushes.

**Enforcement:** Lokale Hooks nutzen den gemeinsamen Prüfvertrag; serverseitige Regeln und der protokollierte Wiederherstellungsweg stehen in `docs/operations/branch-protection.md`. GitHub ist kein Merge-Gate für dieses Repository.
...
- **Dependency versions:** Read `pnpm-workspace.yaml` and `pnpm-lock.yaml`; prose is not a version pin.
