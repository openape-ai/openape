...
## Approved Azure runtime repair — September 24, 2026

Patrick approved implementation of the proposed repair, verification, green-PR merge, signed installation and live IURIO acceptance with “setzen wir das um”. Worktree: `openape-monorepo.worktrees/issue-1377-azure-runtime`, branch `bugfix/issue-1377-azure-runtime`, base `b17e2b54`. This extends milestone 4 of the existing approved plan.

The native reproduction establishes a launcher failure independently of grants: Homebrew az uses Bash/Python; the current executable-only policy returns exit 71. The actual diagnostic also ended with a runtime-authority cancellation after approval; its cause is still unknown and must be reproduced before changing grant behavior.

1. Add an explicit, bounded program runtime descriptor for interpreter executable, fixed arguments, read-only package directories and non-secret runtime environment. Preserve private HOME, network proxy, command grants, resource epochs, executable integrity and fork denial. Route terminal and script calls through the same resolved launch. Existing native programs continue unchanged; replacing an assignment preserves its encrypted login state and refreshes grants.
2. Prepare Azure CLI and the official DevOps extension as code outside encrypted login state. Do not copy host authentication. Verify an empty-HOME az version under the actual native sandbox, denied outside reads/child processes, private-state reuse and tampered executable refusal. Extend existing unit and native suites; no new runner or default test chain.
3. Reproduce the additional grant cancellation, identify its exact cause and fix only proven behavior while retaining revocation enforcement. Cover the failing transition in established authorization tests. Document any remaining provider dependency honestly.
4. Run full lint/typecheck, Pods build and relevant existing tests. Check shared runner tasks before pushes or xcodebuild. Review the issue-linked PR diff; merge only with exact-head CI/E2E/layout green. Run check:ci before signed/notarized distribution, preserve the paired app/profile rollback, install and verify the new app.
5. Reconfigure only the IURIO assignment using the supported MCP; retain its saved private login. Perform a real PR read, adapt the preserved monitor draft to CLI reads, verify complete snapshots and one authorized Telegram baseline message, then enable its existing 900-second schedule. Keep it paused on incomplete reads or uncertain delivery. Never expose tokens or copy owner login stores.

Rollback: revert code before delivery; after installation restore app/profile together only if needed. Do not repeat uncertain Telegram effects. No unrelated Pod, worktree, host configuration or Codex settings are changed.

### Runtime repair progress

- Reproduced Azure launcher exit 71 with the current policy and a clean HOME. An explicit descriptor using the native Python.app executable now passes sandboxed `az version` and `az repos pr list --help`; official azure-devops 1.0.8 is stored separately as code. The bin/python stub also spawns a child, so it cannot be used as the native executable.
- Confirmed the actual runtime grant is `used`/`once`. Brokered status incorrectly excluded consumed one-shot grants, while direct status already handled them. Endpoint regression covers approval, consumption, revocation, denial, expiry and foreign grants; new execution still goes through authoritative consumption. Protocol semantics are unchanged.
- Unit and native runtime checks cover descriptor/executable integrity, protected environment, private-state reuse, denied outside reads and denied fork. Non-secret runtime flags are kept separate from credential redaction to avoid corrupting numeric CLI output. Full final checks, PR, provider deployment, signed installation and real monitoring remain pending.

### Live replacement follow-up

PR 116 merged as `4f5977cb` after exact-source CI/E2E/layout and full local checks passed. The provider and signed/notarized desktop were updated with verified rollback snapshots. Actual run `45f467f2-b8bc-4e19-ae1a-afe0ae93fc27` then exposed historical approval lookup overriding the new owner-assigned grant after an adapter replacement. The executor rejected its stale adapter digest correctly. Prefer the current assignment; consult a historical renewal only if the original is absent, consumed or expired. Keep explicit denial/revocation checks. Two focused signed-grant regressions cover reassignment and continuing renewal. Live account/PR/Telegram acceptance remains required.

### Installed and deployed outcome

PRs 116, 117 and 118 merged with exact-source CI/E2E/layout green and full local checks before delivery. Notarized source `347b869d` is installed with all six Pods preserved and a paired rollback. The current Azure grant now works; private login and PR reads are verified. Owner IdP `prod-8ac60bf0` and Pods provider `prod-4f5977cb` are healthy with unchanged discovery/JWKS. Machine HTTP and broker capacity use the same configured bound of 600; login limits and all grant checks remain intact. The Azure and runtime permissions are reusable approved grants, not repeated owner approvals. Patrick confirmed receipt of the separate Telegram connection test. Full-monitor run `368a4b8e-6320-445e-a841-a7eb352549f5` completed all 39 open PRs, 119 Azure reads and one confirmed Telegram baseline notification. Follow-up `88c09e14-5ff2-49f8-8550-816dc31af162` completed 119 reads with no changes and no message. The Pod is active; its 900-second schedule is enabled at revision 2, checkpoint revision 10. Bounded retries apply only to transient reads. All four milestones are delivered; other Pods and real Codex settings are unchanged.
