# Feature: openape-troop — small cron-driven agents tied to a central SP
...
1. Adds a thin SP at `troop.openape.ai` where the user manages those small agents from any device.
...
`apes agents spawn` keeps creating the full macOS user — but every spawned agent now also enrolls at the troop SP, gets a sync launchd, and picks up cron-scheduled tasks the user defines server-side. Spawn time drops noticeably (no more pi clone + bun install).

**Trigger:** the heavy spawn shape is the only on-ramp today; the user wants a "small cron agent" pattern without abandoning the OS-user model, and the pi dependency feels like ceded control we don't actually need. Troop = the central place to define + steer tasks. The runtime = the small piece that finally makes us self-contained.

---

## Progress

- [ ] Milestone 1: Troop SP scaffold + DB schema + deploy pipeline
- [ ] Milestone 2: SP API endpoints (agent-side + owner-side)
- [ ] Milestone 3: Troop web UI (list agents, manage tasks, view runs)
...
- [ ] Milestone 6: `apes agents spawn` integration — register at troop + install sync launchd; drop pi install path
...
| 2026-05-08 | **Identity reuses `apes agents enroll` model** | Agent gets its own DDISA-agent-account `agent+<name>+<owner-domain>@id.openape.ai` (existing pattern). Troop SP authenticates via that JWT for agent-side endpoints; owner uses their own user JWT for management endpoints. |
| 2026-05-08 | **Extend `apes agents` first, defer dedicated `troop` CLI** | Avoids a parallel CLI surface. Endpoint defaults to `https://troop.openape.ai`, env var `OPENAPE_TROOP_URL` overrides for staging/dev. A `troop` shell alias can be added later as cosmetic sugar. |
| 2026-05-08 | **launchd plists, one per task** | macOS standard. Atomic per-sync swap (drop the whole `~/Library/LaunchAgents/openape.troop.<name>.<task>.plist` set, write the new set, `launchctl bootstrap`/`bootout` to apply). Crontab is deprecated, plus `apes agents spawn` already speaks launchd for the bridge. |
...
| 2026-05-08 | **`apes agents spawn` keeps full-macOS-user behaviour** | New behaviour layered on top: register at troop + install sync launchd. The "lightweight no-OS-user agent" variant is explicitly out-of-scope for v1. |
| 2026-05-08 | **Multi-host: 1:1 agent ↔ host, hostId via Mac-Hardware-UUID** | Each `apes agents spawn` produces a new agent identity bound to the host it ran on. IdP rejects email collisions, so a second host has to use a different agent name (`alice-laptop`, `alice-mini`). At first sync the agent reports `hostname` (mutable, human-readable) plus `hostId` derived from `IOPlatformUUID` (stable, hardware-rooted). Troop pins `hostId` after first sync — subsequent syncs from a different `hostId` for the same agent JWT are rejected with 401. That's the cheap "the keypair was copied to another machine" alarm. |
| 2026-05-08 | **Troop runtime replaces `@mariozechner/pi-coding-agent`** | Today `apes agents spawn --bridge` clones pi, `bun install`s it, writes a litellm extension, and chat-bridge runs `pi --mode rpc` per room. With our own runtime in M5 we can offer the same RPC contract (`{ session_id, user_msg }` in, `text_delta`/`done` events out) — chat-bridge swaps subprocess from pi to `apes agents serve --rpc`. Eliminates pi clone + bun install + extension write from the spawn flow, and gives us full control over the tool spec + streaming events. |
...
- **`apes agents enroll`** (`packages/apes/src/lib/agent-bootstrap.ts`) issues an Ed25519 keypair, registers the agent at the IdP (`/api/admin/agents`), gets back an agent JWT with `act:'agent'`. We reuse this verbatim — every troop agent IS a DDISA agent.
- **`@openape/cli-auth`** (`~/.config/apes/auth.json`) holds the owner's IdP token. Troop is just another SP that exchanges it for a troop-scoped token via the standard `/api/cli/exchange` flow already used by `ape-plans`/`ape-tasks`.
...
- **`apes agents spawn`'s launchd writer** (`packages/apes/src/lib/llm-bridge.ts:bridgePlistPath` etc) — there's an existing template for building plist files. Reuse the helper, point it at the troop sync + run scripts instead of the bridge.

### What's actually new

- A new app `apps/openape-troop` (Nuxt + Drizzle, mirrors `apps/openape-plans` shape) at `troop.openape.ai`
...
- **Troop SP compromised** → attacker can edit task system-prompts and tool lists. Agent's tool registry is built-in (bounded); attacker can't smuggle a brand-new "delete-everything" tool. Worst case: phishing-via-system-prompt, or driving a tool we shipped to do something the owner didn't intend.
...
```
┌──────────────────────────────────────────────────────────────┐
│  troop.openape.ai (apps/openape-troop — Nuxt SP)             │
│                                                              │
│  Web UI                                                      │
...
│   ├─ openape.troop.sync.plist           every 5min, calls     │
│   │                                     `apes agents sync`    │
│   └─ openape.troop.<task>.plist (×N)    on cron, calls        │
│                                         `apes agents run`     │
│                                                                │
│  ~/.openape/agent/                                             │
...
**Goal:** `https://troop.openape.ai` returns 200 logged-out, 302 to login when accessed without a session, schema migrations idempotently bring up three tables.

**Files to create:**
- `apps/openape-troop/` — Nuxt 4 app, copy structure from `apps/openape-plans/app/`
  - `package.json` — `@openape/nuxt-auth-sp@workspace:*`, `drizzle-orm`, `@libsql/client`
  - `nuxt.config.ts` — `openapeSp: { spName: 'OpenApe Troop', clientId: 'troop.openape.ai', fallbackIdpUrl: 'https://id.openape.ai' }`
  ...
  - `app/pages/index.vue` — temporary "troop is live" landing page
- `.github/workflows/deploy-troop.yml` — copy `deploy-chat.yml`, swap paths/service-name/health-URL
- `scripts/deploy-troop.sh` — copy `scripts/deploy-chat.sh`, swap `openape-troop`
...
// reach troop — owner picks distinct names like `alice-laptop` /
...
**Acceptance:**
- [ ] `pnpm turbo run build --filter=openape-troop` succeeds
- [ ] `node .output/server/index.mjs` locally serves `/login` (HTTP 200)
- [ ] After deploy: `curl -o /dev/null -w '%{http_code}' https://troop.openape.ai/` → 200, `/login` → 200
...
**Files:**
- `apps/openape-troop/server/api/agents/`
...
- `apps/openape-troop/server/api/agents/me/`
...
- `apps/openape-troop/server/utils/auth.ts` — two resolvers:
...
**Tools allowlist:** server-side validation that `tools[]` contains only known names. Read from a JSON manifest committed in the repo, e.g. `apps/openape-troop/server/tool-catalog.json`. The runtime registry on the agent side has the actual implementations; the SP just enforces "you can't add a tool we don't know exists". Same catalog file is served to the web UI for the picker.

**Acceptance:**
- [ ] `curl -H "Authorization: Bearer $USER_JWT" troop.openape.ai/api/agents` returns `[]` initially, then a row after `agents/me/sync` is called
- [ ] `curl -H "Authorization: Bearer $AGENT_JWT" troop.openape.ai/api/agents/me/tasks` returns `[]` for a fresh agent
...
## Milestone 3 — Troop web UI

**Goal:** Owner can manage agents end-to-end in the browser without curl.

**Files:**
- `apps/openape-troop/app/pages/agents/index.vue`
...
- `apps/openape-troop/app/pages/agents/[name].vue`
...
- `apps/openape-troop/app/pages/login.vue` — `<OpenApeOAuthErrorAlert />` with troop-specific copy
- `apps/openape-troop/app/components/CronInput.vue` — input + live "next run at" preview using a tiny cron parser (`cron-parser` ~5KB)
- `apps/openape-troop/app/components/ToolPicker.vue` — fetches the catalog, multi-select with descriptions
...
**Goal:** Running `apes agents sync` on an agent's macOS user reconciles `~/Library/LaunchAgents/openape.troop.*.plist` with the server-side task list.

**Files:**
- `packages/apes/src/commands/agents/sync.ts` — new citty command
- `packages/apes/src/lib/troop-client.ts` — typed wrapper around troop API (sync, tasks, runs)
...
5. Generate a launchd plist per task at `~/Library/LaunchAgents/openape.troop.<agent_name>.<task_id>.plist`. plist runs `apes agents run <task_id>` with `StartCalendarInterval` derived from the cron string.
6. Diff: existing plists in `~/Library/LaunchAgents/openape.troop.<agent_name>.*` minus desired plists → `launchctl bootout` + `rm`
...
**Acceptance:**
- [ ] On a fresh agent host, after creating one task in the web UI, `apes agents sync` writes one plist and `launchctl print gui/$UID/openape.troop.alice.mail-triage` prints the job
...
**Goal:** Running `apes agents spawn alice` from a fresh shell produces a fully troop-connected agent. Sync launchd is installed, ready to pick up tasks the owner defines server-side.
...
- `packages/apes/src/commands/agents/spawn.ts` — change step list:
  ...
  3. **new:** call troop `/api/agents/me/sync` once with the agent JWT to register the agent at the SP (creates the row with hostname + hostId + pubkey)
  4. **new:** install `~/Library/LaunchAgents/openape.troop.sync.plist` running `apes agents sync` every 300 seconds (`StartInterval: 300`)
  5. **new:** `launchctl bootstrap gui/$UID` the sync plist so it starts running immediately
- `packages/apes/src/lib/troop-bootstrap.ts` — helper for steps 3+4 (mirrors `lib/llm-bridge.ts` shape)
...
**Visible change for the user:** the spawn output's final summary now includes a "🔗 Agent registered at https://troop.openape.ai/agents/<name>" line. Spawn time drops noticeably (no more 30s pi download + bun install).

**Acceptance:**
- [ ] After `apes agents spawn alice`, the owner sees `alice` in `https://troop.openape.ai/agents` within ~10 seconds (one sync cycle)
- [ ] `launchctl print gui/$UID/openape.troop.sync` shows the sync job loaded and active
...
**Goal:** `https://troop.openape.ai` is publicly reachable, has a valid cert, and one real end-to-end agent (mine, `daily-summary`) runs against it.

**Steps:**
1. **DNS:** `exo dns add A openape.ai -n troop -a 85.217.175.26` (chatty's IP, where the other Nuxt SPs sit)
...
4. **First deploy** via the new `deploy-troop.yml` workflow (push to main triggers, change-detection on `apps/openape-troop/**`)
5. **Smoke test:** `curl https://troop.openape.ai/` → 200; login flow round-trips through id.openape.ai and lands back on `/agents`
...
- **Troop SP outage = no new task pickup.** Already-installed launchd plists keep running with their last-known spec (they read from local cache `~/.openape/agent/tasks/`). New tasks/edits don't propagate until the SP is back. Acceptable for a personal-tool SP.
...
- **Troop CLI alias** (`troop spawn …`) — defer until `apes agents` is mature; symbolic alias is trivial when the time comes.
- **Web push from troop** — getting an iOS push when an agent run errored. Nice-to-have, not blocking. Could reuse the chat-bridge VAPID infra.
...
```bash
# Build & boot troop locally
cd apps/openape-troop && pnpm dev
open http://localhost:3001

# Probe API endpoints (replace tokens)
USER_JWT=$(jq -r .id_token ~/.config/apes/sp-tokens/troop.openape.ai.json)
...
apes agents sync --troop-url http://localhost:3001  # (env-var alternative: OPENAPE_TROOP_URL)
ls ~/Library/LaunchAgents/openape.troop.*.plist
