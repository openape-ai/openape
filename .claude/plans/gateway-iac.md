...
- **M4 — `_MODELS`-Entkopplung + lindeverlag stilllegen** — ✅ **LIVE + verifiziert (PR #802 repo, deployed 2026-06-19 via `deploy:gateway --deploy`)**. Completion-Beweis je Account (echte DDISA-Token-Smokes über die Edge): **default `/v1` ungated → 200** `LocalCore-Instant` (Owner-Token OHNE „default"-Grant kommt durch = Ungating bewiesen); **delta-mind granted → 200** `delta-mind/gpt-5.5`; **lindeverlag retired → 400** „Invalid model name". codex-proxy-Orphan entfernt, Drift-Guard „in sync". **Deploy-Gotcha (gefixt, PR #807):** `compose up -d` recreatet litellm NICHT bei reiner Mount-Config-Änderung → litellm lief erst mit alter Policy weiter (manuell `restart` nötig); Script jetzt `--force-recreate --remove-orphans`.
