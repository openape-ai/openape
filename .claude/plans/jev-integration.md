...
<div class="card"><h3>App Settings</h3><p><strong>TypeSafe AI - Jev - API Key</strong></p><p>•••••••••••••••</p><p><strong>Connect or replace API key</strong></p></div>

Owner correction (September 25): the Jev setup consists only of the field label, masked input and connect/replace button directly in App settings. Remove explanatory copy, account-creation links and Jev panels from Script and Permissions. Explanations belong in help and documentation. Codex chooses between ordinary code, Jev and LLM calls from the installed runtime reference and configures Pod assignments through the resources API; UI copy is not its decision input.
...
- Place the compact optional TypeSafe field directly in App settings with a masked input and connect/replace button. No explanatory text, account-creation link or disconnect button is shown there. Jev is not a new prerequisite for completing onboarding.
- Verify with authenticated model discovery. Display actionable errors when setup fails; report invalid key, unavailable network and temporary provider failure distinctly; a timeout does not prove that a key is invalid. Do not fabricate an account email because model discovery does not return one.
...
- For missing setup, Codex creates the useful draft and directs the owner to protected key entry in App settings. It may configure the connection reference under existing administration authority; no extra blanket approval queue is introduced. Existing execution grants still apply.
- Document the Jev helper in the handbook and installed agent runtime reference. Do not add Jev panels to Script or Permissions. The official TypeSafe agent skill is useful background, but a global third-party skill installation is not a prerequisite. OpenApe's installed runtime reference is authoritative for its helper and permissions.
...
- 2026-09-25 UI correction: compact TypeSafe field moved directly to App settings; explanatory copy, create-key link, disconnect control and native/central Jev panels removed. Codex discovery and scoped resources API remain available independently of UI text. Full lint (54 tasks), typecheck (77 tasks), Pods build, 34 focused tests and five browser tests pass. Four screenshots were inspected and published in the [updated UI report](https://testrun.openape.ai/r/sDUdikwIBcms_bwtddTTcAJG). The existing development key is preserved.
...
- M3: installed MCP reference, internal authoring instructions, Pod inspection, native key setup, central projection, installed agent reference, run metadata and bilingual handbook updated. Full and format-2 central projections retain only availability metadata. Native account UI and narrow central screenshots were inspected.
...
- Delivery: [native PR #134](https://repos.openape.ai/patrick/monorepo/pulls/134), implementation commit `fa31c9bc684ed02b6c23a5f2d762eba62daf961a`, target `63ab0bf0788e02dfd5c6fd8f5f7936977cfbbe6f`. The native PR diff is mergeable and was reviewed. [Published evidence](https://testrun.openape.ai/r/q8kG0M6twEJoIpqcVVx8reEg) includes four inspected screenshots. Commit and push hooks passed the affected unit contract, including the relay consumer. External CI passed (7m 2s) for final PR head `9f3ee93458b21d8e9415e4d99fd1d0f4c95c3038` ([run 5113](https://git.openape.ai/openape-ai/openape/actions/runs/5113/jobs/0)). The final native diff was rechecked against target `63ab0bf0788e02dfd5c6fd8f5f7936977cfbbe6f`; no conflicts or whitespace errors. The final development build was restarted and its account UI confirms TypeSafe is still Connected.
