...
- September 20, 2026: Patrick requested Codex-style model selection through `/` in the chat input. Verified the [official slash-command reference](https://learn.chatgpt.com/docs/reference/slash-commands): type `/`, filter commands and use `/model`. Moved the existing selector into the composer and added local keyboard/search interactions without changing model availability or runtime authority. The existing component suite covers draft preservation, no command submission, literal paths, cancellation and running-turn locking; native coverage checks actual provider model IDs and narrow German layout. Canonical main advanced to `403ecf68` (native issue authority); preserved both active-work sections while integrating it. Current issue: https://repos.openape.ai/patrick/monorepo/issues/1359. Final source checks and evidence are recorded in PR 78.
