...
async function handleAddSshKey() {
  ...
  try {
    ...
    error.value = err?.data?.title ?? 'Failed to add SSH key'
  }
  finally {
...
async function handleDeleteSshKey(keyId) {
  ...
  try {
    ...
    error.value = err?.data?.title ?? 'Failed to remove SSH key'
  }
}
            ...
            <tbody class="divide-y divide-(--ui-border)">
              <tr v-for="k in sshKeys" :key="k.keyId" class="odd:bg-(--ui-bg-elevated)/40 even:bg-(--ui-bg) hover:bg-(--ui-bg-elevated)">
                ...
                <td class="px-4 py-3 text-right">
                  <UButton variant="ghost" size="xs" color="error" @click="handleDeleteSshKey(k.keyId)">
                    Remove
                  </UButton>
                </td>
              </tr>
