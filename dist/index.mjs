 ...
 * S3-compatible unstorage driver — works with Exoscale SOS, MinIO, etc.
 * Fork of unstorage v1.17.4 s3 driver with XML prolog fix.
 */
...
export default defineDriver((options) => {
  ...
  const awsFetch = async (fetchUrl, opts) => {
    const request = await getAwsClient().sign(fetchUrl, opts);
    const res = await fetch(request);
    if (!res.ok) {
      if (res.status === 404) return null;
      throw createError(DRIVER_NAME, `[${request.method}] ${fetchUrl}: ${res.status} ${res.statusText} ${await res.text()}`);
    }
    return res;
  };
  ...
  return {
    ...
    setItem: async (key, value) => { await putObject(key, value); },
    setItemRaw: async (key, value) => { await putObject(key, value); },
    removeItem: async (key) => { await deleteObject(key); },
    getMeta: async (key) => {
...
function parseList(xml) {
  ...
  if (!listBucketResult) {
    if (!xml.includes("<")) throw new Error("Invalid response from S3: not XML");
    const errorMsg = xml.match(/<Message>([\s\S]*?)<\/Message>/)?.[1];
    if (errorMsg) throw new Error(`S3 error: ${errorMsg}`);
    throw new Error("Missing <ListBucketResult>");
  }
  const contents = listBucketResult.match(/<Contents[^>]*>([\s\S]*?)<\/Contents>/g);
  if (!contents?.length) return [];
  return contents.map((content) => content.match(/<Key>([\s\S]+?)<\/Key>/)?.[1]).filter(Boolean);
}
